...
subtitle: "2013-2014 Data Analyses"
author:
...
##  Student data preparation issues and requirements from previous years
...
The output from this was then manually added to the SGPstateData.R file and [committed to Github](https://github.com/CenterForAssessment/SGPstateData/commit/403b3942caf51bec7aef578af092ea964f7c642d).  This process can be facilitated by the authors of the `SGP` package [upon request](https://github.com/CenterForAssessment/SGPstateData/issues).  Note that these knots and boundaries were not used in the 2014 analyses because this was the first year in which the Analytic Geometry test was administered.  Given that they were not used, and the fact that this calculation ***only a single year of data***, it may be necessary to recalculate them next year and change them in `SGPstateData` if those values are quite different (small changes would likely not have a large impact on analyses going forward).
...
The `SGPstateData` file also includes a lookup table that allows various functions in the SGP package to translate between the naming conventions used within them and the variable names GaDOE uses.  The 2014 data included an identifier for gifted and talented students, which will be used in result summarizations.
...
The `GA_SGP_Norm_Group_Preference` should be submitted to the [SGP Package maintainers](https://github.com/CenterForAssessment/SGP#contributors) for inclusion in the package so that it is contained in `SGPstateData` when the package is loaded.^[Alternatively, this `R` object can be directly embedded into the appropriate list slot in the `SGPstateData` object manually.  This process is demonstrated in Step 6 below, but is not recommended].
...
The CRCT grade level ELA, Reading, Math, Science and Social Studies analyses were conducted separately from the EOCT course analyses.  Mainly this is done because the CRCT analyses do not require custom configuration scripts like EOCT does.  Given the simplistic use of only the same test as prior test scores in these analyses, the SGP package's functions can identify the analyses to be conducted internally.  This also allows the analyses to be completed and checked, and interim results saved which is sometimes necessary due to the computationally intensive nature of the analyses, particularly with a state the size of Georgia.^[Note that Science was analyzed along with the EOCT courses in 2013.  This was done because science growth projections can follow two different paths: the first projecting from 8th grade science to Biology, and the other to Physical Science (see the added `sgp.projection.sequence` [line in the configuration script](https://github.com/CenterForAssessment/Georgia/blob/master/SGP_CONFIG/EOCT/2014/SCIENCE.R#L16)).  This dual projection process may be a part of future analyses.]
...
The EOCT content area progressions specific analyses were then calculated using the configuration scripts.  As with the CRCT analyses, cohort referenced SIMEX corrected versions of all SGPs were eventually produced^[Unlike the CRCT, EOCT cohort referenced SGPs use only 2 prior years of data as specified in the configuration scripts.] by specifying both `sgp.calculate.simex = TRUE` and `sgp.calculate.simex.baseline = TRUE` in each configuration code chunk.
...
Georgia_SGP <- analyzeSGP(
		...
		sgp.projections.lagged.baseline = FALSE,
		simulate.sgps = TRUE,   # Needed for SGP_STANDARD_ERROR and SGP_BASELINE_STANDARD_ERROR
		parallel.config = list(BACKEND='PARALLEL', WORKERS=list(PERCENTILES=1, BASELINE_PERCENTILES=1, TAUS=13, SIMEX=5))) # Parallel config as run for official results
...
#  Add Norm Group Preferences if they have not been submitted to SGPstateData (NOT RECOMENDED!): 
...
- Baseline matrices.  Number of prior scores required will have to be established first.
	* Unadjusted version created first
  * SIMEX adjusted versions (require unadjusted matrices)
- Misc code adjustments to switch to cohort ref'd & cohort SIMEX
