...
```{r, echo=FALSE, include=FALSE}
  ...
  knitr::opts_chunk$set(cache.path = "_cache/GA_SGP_2019", fig.path="../img/Tech_Reports/2019/")
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_EOG <- table(Georgia_SGP@Data[YEAR=='2019' & CONTENT_AREA %in% GL_subjects & VALID_CASE=='VALID_CASE', GRADE, CONTENT_AREA])
```
```{r, results='asis', echo=FALSE, N_tableEOG}
  ...
  cat(dualTable(as.matrix(n_tbl_EOG_B), 
  	align=paste(rep('r', dim(n_tbl_EOG_B)[2]), collapse=''),
		...
		caption='Number of Valid EOG Student Records by Grade and Subject for 2019'))
```
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_EOC <- table(Georgia_SGP@Data[YEAR=='2019' & CONTENT_AREA %in% EOC_subjects & VALID_CASE=='VALID_CASE', GRADE, CONTENT_AREA])
```
```{r, results='asis', echo=FALSE, N_tableEOC}
  ...
  cat(dualTable(as.matrix(n_tbl_EOC_B), align=paste(rep('r', dim(n_tbl_EOC_B)[2]), collapse=''), 
		caption='Total Number of Valid EOC Student Records by Subject for 2019'))
...
As an example, Figure `r getCounter("figure")+1` shows the results for 8<sup>th</sup> grade ELA as an example of good model fit.  Figure `r getCounter("figure")+2` is an example of minor model misfit from the Geometry "same-year" course repeater analysis (i.e. students have two Geometry scores in 2019).
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, GOFplots}
  ...
  dat <- Georgia_SGP@Data[SGP_NORM_GROUP %in% "2017/ELA_6; 2018/ELA_7; 2019/ELA_8", 
	...
	gofSGP(dat, state="GA", years='2019', content_areas="ELA", use.sgp="SGP", ceiling.floor= FALSE, output.format="PNG")
	gofSGP(dat, state="GA", years='2019', content_areas="ELA", use.sgp="SGP_SIMEX", ceiling.floor= FALSE, output.format="PNG") # Run for TAC presentation
	gofSGP(dat, state="GA", years='2019', content_areas="ELA", use.sgp="SGP_SIMEX_RANKED", ceiling.floor= FALSE, output.format="PNG") # Run before SIMEX and Rename folder???

  dat <- Georgia_SGP@Data[SGP_NORM_GROUP %in% "2019/GEOMETRY_EOCT; 2019/GEOMETRY_EOCT",
  				 c("VALID_CASE", "CONTENT_AREA", "GRADE", "YEAR", "ID", "SGP", "SGP_SIMEX", "SGP_SIMEX_RANKED", "SCALE_SCORE", "SCALE_SCORE_PRIOR", "SGP_NORM_GROUP"), with=FALSE]

	gofSGP(dat, state="GA", years='2019', content_areas='GEOMETRY', use.sgp="SGP", ceiling.floor= FALSE, output.format="PNG")
	gofSGP(dat, state="GA", years='2019', content_areas='GEOMETRY', use.sgp="SGP_SIMEX", ceiling.floor= FALSE, output.format="PNG")   #   Run for TAC presentation
	gofSGP(dat, state="GA", years='2019', content_areas='GEOMETRY', use.sgp="SGP_SIMEX_RANKED", ceiling.floor= FALSE, output.format="PNG") # Rename folder???
...
```{r, results="asis", echo=FALSE, G8_ELA_GoF_Cohort}
		placeFigure(page.break= TRUE, pdf.width = 0.875,
			files = "../img/Goodness_of_Fit/ELA.2019/gofSGP_Grade_8.png",
			caption = "Goodness of Fit Plot for 2019 ***Uncorrected*** 8<sup>th</sup> Grade ELA:  Example of good model fit.")
```
...
```{r, results="asis", echo=FALSE, Geom_GoF_Cohort}
		placeFigure(# page.break= TRUE,
			files = "../img/Goodness_of_Fit/GEOMETRY.2019/2019_GEOMETRY_EOCT;2019_GEOMETRY_EOCT.png",
			caption = "Goodness of Fit Plot for a 2019 ***Uncorrected*** Geometry Repeater Progression: Example of slight model misfit.")
```


###  SIMEX Corrected model fit

The basic SIMEX model fit ***is not*** expected to be uniformly distributed regardless of prior achievement.  In these models we expect misfit in the form of increased high SGPs for students with lower prior performance (and a complementary decrease in low SGPs for those students), and the reverse expectation for high achieving students.  The model changes are visible in the goodness of fit plots in Figure `r getCounter("figure")+1` where the basic SIMEX correction method has been applied to the 8<sup>th</sup> grade ELA model, and Figure `r getCounter("figure")+2` for the same Geometry progression as presented above.


```{r, results="asis", echo=FALSE, G8_ELA_GoF_SIMEX}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/ELA.2019.SIMEX/gofSGP_Grade_8.png",
			caption = "Goodness of Fit Plot for 2019 ***Basic SIMEX Corrected*** 8<sup>th</sup> Grade ELA:  Example of good model fit.")
```
...
```{r, results="asis", echo=FALSE, Geom_GoF_SIMEX}
		placeFigure(
			files = "../img/Goodness_of_Fit/GEOMETRY.2019.SIMEX/2019_GEOMETRY_EOCT;2019_GEOMETRY_EOCT.png",
			caption = "Goodness of Fit Plot for a 2019 ***Basic SIMEX Corrected*** Geometry Progression: Example of slight model misfit.")
```
...
```{r, results='asis', echo=FALSE, Student_EOG}
	student.cor.grd <- Georgia_SGP@Data[YEAR=='2019' & VALID_CASE=='VALID_CASE' & CONTENT_AREA %in% GL_subjects][, list(
...
The relationships between growth and prior achievement reported in Table `r tblNum(1)` are nearly non-existent for cohort referenced SGPs and slightly negative after SIMEX correction.  These results are as expected for appropriate fit to the respective models as discussed in the EOG section above.

```{r, cache=TRUE, echo=FALSE, include=FALSE, Student_EOCT}
	student.cor.eoct <- Georgia_SGP@Data[YEAR=='2019' & VALID_CASE=='VALID_CASE' & CONTENT_AREA %in% EOC_subjects][, list(
		`$\\\rr_ {  Test Scores}$` = round(cor(SCALE_SCORE, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 2), 
...
```{r, results='asis', echo=FALSE, Student_EOCT_1}
	eoct_tmp_tbl <- student.cor.eoct[!is.na(student.cor.eoct[["$\\\rr_ {  Test Scores}$"]])] # & CONTENT_AREA %in% c("GRADE_9_LIT", "AMERICAN_LIT") # Don't need to break up into two tables in 2019

	setkey(eoct_tmp_tbl, CONTENT_AREA, Most_Recent_Prior)
...
```{r, results='asis', echo=FALSE, SumEOG}
	EOG_smry <- Georgia_SGP@Data[CONTENT_AREA %in% GL_subjects & YEAR=='2019'][, list(MEDIAN = median(as.numeric(SGP), na.rm=TRUE), MEAN = round(mean(SGP, na.rm=TRUE), 1)), by=c('CONTENT_AREA', 'GRADE')][!is.na(MEDIAN)]
	setkey(EOG_smry)
  ...
  cat(dualTable(as.matrix(EOG_smryB), title="", n.cgroup=c(1, dim(EOG_smryB)[2]-1), cgroup=c("", "Grades"),
  	align=paste(rep('r', dim(EOG_smryB)[2]), collapse=''),
		caption='2019 EOG Median (Mean) Student Growth Percentile by Grade and Content Area.'))
```

```{r, results='asis', echo=FALSE, SumEOC}
	EOC_smry <- Georgia_SGP@Data[CONTENT_AREA %in% EOC_subjects & YEAR=='2019'][,list("Median SGP"=median(as.numeric(SGP), na.rm=TRUE), "Mean SGP"=round(mean(SGP, na.rm=TRUE), 1)), by='CONTENT_AREA']
	setkey(EOC_smry)
  ...
  cat(dualTable(as.matrix(EOC_smry_B), align='rcc',
		caption='2019 EOC Median and Mean Student Growth Percentile by Content Area.'))
```
...
```{r, results='asis', echo=FALSE, SIMEX_SumEOG}
	EOG_SIMEX_smry <- Georgia_SGP@Data[CONTENT_AREA %in% GL_subjects & YEAR=='2019'][,list(MEDIAN=median(as.numeric(SGP_SIMEX_RANKED), na.rm=TRUE), MEAN=round(mean(SGP_SIMEX_RANKED, na.rm=TRUE), 1)), by=c('CONTENT_AREA', 'GRADE')][!is.na(MEDIAN)]
  ...
  cat(dualTable(as.matrix(EOG_SIMEX_smry_B), title="", n.cgroup=c(1, dim(EOG_SIMEX_smry_B)[2]-1), cgroup=c("", "Grades"),
  	align=paste(rep('r', dim(EOG_SIMEX_smry_B)[2]), collapse=''),
		caption='Ranked SIMEX Corrected EOG Median (Mean) Student Growth Percentile by Grade and Content Area for 2019'))
```


```{r, results='asis', echo=FALSE, SIMEX_SumEOC}
	EOC_SIMEX_smry <- Georgia_SGP@Data[CONTENT_AREA %in% EOC_subjects & YEAR=='2019'][,list("Median SGP"=median(as.numeric(SGP_SIMEX_RANKED), na.rm=TRUE), "Mean SGP"=round(mean(SGP_SIMEX_RANKED, na.rm=TRUE), 1)), by='CONTENT_AREA']
  ...
  cat(dualTable(as.matrix(EOC_SIMEX_smry_B), align='rcc',
		caption='Ranked SIMEX Corrected EOC Median (Mean) Student Growth Percentile by Content Area for 2019'))
```
...
```{r, results="asis", echo=FALSE, ELA_Bubble}
		placeFigure(
			files = "../img/Bubble_Plots/2019/State/Style_1/Georgia_2019_ELA_State_Bubble_Plot_(Prior_Achievement).png",
			caption = "School-level Bubble Plots for Georgia: ELA, 2018-2019 ($N \\geq 15$).", pdf.width = 0.925)
```
...
```{r, results="asis", echo=FALSE, Math_Bubble}
		placeFigure(
			files = "../img/Bubble_Plots/2019/State/Style_1/Georgia_2019_Mathematics_State_Bubble_Plot_(Prior_Achievement).png",
			caption = "School-level Bubble Plots for Georgia: Mathematics, 2018-2019 ($N \\geq 15$).", pdf.width = 0.925)
```
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	sch.msgp <- Georgia_SGP@Summary$SCHOOL_NUMBER$SCHOOL_NUMBER__YEAR__SCHOOL_ENROLLMENT_STATUS[YEAR %in% c('2017', '2018', '2019') & !is.na(SCHOOL_NUMBER) & MEDIAN_SGP_COUNT > 14] # Last 3 YEARs
	sch.msgp.subj <- Georgia_SGP@Summary$SCHOOL_NUMBER$SCHOOL_NUMBER__CONTENT_AREA__YEAR__SCHOOL_ENROLLMENT_STATUS[YEAR %in% c('2017', '2018', '2019') & !is.na(SCHOOL_NUMBER) & MEDIAN_SGP_COUNT > 14] # Last 3 YEARs
	sch.msgp.grd <- Georgia_SGP@Summary$SCHOOL_NUMBER$SCHOOL_NUMBER__CONTENT_AREA__YEAR__GRADE__SCHOOL_ENROLLMENT_STATUS[YEAR == '2019' & !is.na(SCHOOL_NUMBER) & MEDIAN_SGP_COUNT > 14]
	...
	sch.cor <- sch.msgp[, list(
			MEDIAN_x_Uncorrected = round(cor(MEDIAN_SGP, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
			MEAN_x_Uncorrected=round(cor(MEAN_SGP, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
			MEDIAN_x_Ranked_SIMEX = round(cor(MEDIAN_SGP_SIMEX_RANKED, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2), 
			MEAN_x_Ranked_SIMEX = round(cor(MEAN_SGP_SIMEX_RANKED, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2), N = .N), keyby = 'YEAR']
	...
	sch.cor.subj <- sch.msgp.subj[, list(
			...
			MEAN_x_Uncorrected = round(cor(MEAN_SGP, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
			MEDIAN_x_Ranked_SIMEX = round(cor(MEDIAN_SGP_SIMEX_RANKED, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
			MEAN_x_Ranked_SIMEX = round(cor(MEAN_SGP_SIMEX_RANKED, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2), N = .N), keyby = c('CONTENT_AREA', 'YEAR')]

	## 2019 by grade - School_Grade_EOG_Correlations
	sch.cor.grd <- sch.msgp.grd[, list(
		MEDIAN_x_Uncorrected = round(cor(MEDIAN_SGP, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
		MEAN_x_Uncorrected = round(cor(MEAN_SGP, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
		MEDIAN_x_Ranked_SIMEX = round(cor(MEDIAN_SGP_SIMEX_RANKED, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
		MEAN_x_Ranked_SIMEX = round(cor(MEAN_SGP_SIMEX_RANKED, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2), N = .N), keyby = c('CONTENT_AREA', 'GRADE')]
...
```{r, results='asis', echo=FALSE, School_Cor_Grand}
	tmp_capt <- "2017 to 2019 Correlations between Mean Prior Standardized Scale Score and Aggregate SGPs - (Combined Subjects)"
	...
	n_col_group <- c(1,4,1)  # Just aggregate SGP groups.
...
```{r, results='asis', echo=FALSE, School_EOG_Correlations}
	tmp.cap <- "2017 to 2019 School Level EOG Correlations between Mean Prior Standardized Scale Score and Aggregate SGPs by Content Area."	
	...
	setnames(tmp_tbl, gsub(" X ", "\n", sapply(names(tmp_tbl), capwords)))
...
```{r, results='asis', echo=FALSE, School_Grade_EOG_Correlations}
	tmp.cap <- "2019 School Level EOG Correlations between Mean Prior Standardized Scale Score and Aggregate SGPs by Content Area and Grade."
...
```{r, results='asis', echo=FALSE, School_EOC_Cor}
	tmp.cap <- "2017 to 2019 School Level EOC Correlations between Mean Prior Standardized Scale Score and Aggregate SGPs by Content Area."
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, Target_Sum}
	target_smry <- Georgia_SGP@Data[,list( # [YEAR=='2019' & CONTENT_AREA %in% GL_subjects]
...
```{r, echo=FALSE, include=FALSE, cache=TRUE, Box_Plots, fig.height=8, fig.width=6}
	dir.create("../img/Tech_Reports/2019/", recursive = TRUE, showWarnings = FALSE)
...
```{r, results="asis", echo=FALSE, EOG_BoxPlots}
		placeFigure(
			files = "../img/Tech_Reports/2019/Box_Plots-1.png", page.break= TRUE,
...
```{r, results="asis", echo=FALSE, EOC_BoxPlots}
		placeFigure(
			files = "../img/Tech_Reports/2019/Box_Plots-2.png", page.break= TRUE,
			caption = "English Language Arts EOC 2019 growth target box plots by current achievement level.")

		placeFigure(
			files = "../img/Tech_Reports/2019/Box_Plots-3.png", page.break= TRUE,
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, Target_Cor}
	target_cor <- Georgia_SGP@Data[,list( # [YEAR=='2019' & CONTENT_AREA %in% GL_subjects]
...
```{r, echo=FALSE, include=FALSE, cache=TRUE, Scatter_Plot2, fig.height=3.75, fig.width=5.5}
	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			...
			labs(y = "2019 Target", x = "Current Grade Level Math Scale Score") + theme(axis.title=element_text(size=10, face="bold"), legend.position="bottom") + 
	...
	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			...
			labs(y = "2019 Target", x = "Grade 9 Lit Scale Score") + theme(axis.title=element_text(size=10, face="bold"), legend.position="bottom") + 
	...
	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			...
			labs(y = "2019 Target", x = "Algebra I Scale Score")  + theme(axis.title=element_text(size=10, face="bold"), legend.position="bottom") + 
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") + guides(col=guide_legend("Achievement Levels:", size = 6, title.theme = element_text(size = 8, face = "bold")))
	...
	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			...
			labs(y = "2019 Target", x = "Coordinate Algebra Scale Score") + theme(axis.title=element_text(size=10, face="bold"), legend.position="bottom") + 
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") + guides(col=guide_legend("Achievement Levels:", size = 6, title.theme = element_text(size = 8, face = "bold")))
```

```{r, results="asis", echo=FALSE, EOG_ScatterPlots}
		placeFigure(
			files = "../img/Tech_Reports/2019/Scatter_Plot1-1.png", pdf.width = 0.95,
			caption = "ELA 2019 growth targets by current scale score.")

		placeFigure(
			files = "../img/Tech_Reports/2019/Scatter_Plot2-1.png", pdf.width = 0.95, page.break= TRUE,
			caption = "Mathematics 2019 growth targets by current scale score.")
```
...
```{r, results="asis", echo=FALSE, EOC_ScatterPlots}
		placeFigure(
			files = "../img/Tech_Reports/2019/Scatter_Plot1-2.png", pdf.width = 0.95,
			caption = "Grade 9 Lit 2019 growth targets by current scale score.")

		placeFigure(
			files = "../img/Tech_Reports/2019/Scatter_Plot2-2.png", pdf.width = 0.95, page.break= TRUE,
...
######
		...
		placeFigure(
			files = "../img/Tech_Reports/2019/Scatter_Plot1-3.png", pdf.width = 0.95,
			caption = "2019 Algebra I growth targets by current scale score.")

		placeFigure(
			files = "../img/Tech_Reports/2019/Scatter_Plot2-3.png", pdf.width = 0.95,
...
######
		...
		placeFigure(
			files = "../img/Tech_Reports/2019/Scatter_Plot1-4.png", pdf.width = 0.95,
			caption = "Coordinate Algebra 2019 growth targets by current scale score.")

		placeFigure(
			files = "../img/Tech_Reports/2019/Scatter_Plot2-4.png", pdf.width = 0.95, page.break= TRUE,
