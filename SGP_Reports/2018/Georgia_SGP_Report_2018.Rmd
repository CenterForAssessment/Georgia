...
```{r, results='asis', echo=FALSE, Target_Sum_EOG}
	...
	for (ca in GL_subjects) {
		...
		for (achlev in c("Beginning Learner", "Developing Learner", "Proficient Learner", "Distinguished Learner")) {
			...
			names(tmp_EOG_smry) <- c("Target Content Area", "Achievement Level", t(as.matrix(tmp_data[, list(Target)])))
  ...
  cat(dualTable(as.matrix(target_smry_A), title="", n.cgroup=c(1, 1, dim(target_smry_A)[2]-2), cgroup=c("", "", "Growth Targets"),
  	align=paste(rep('r', dim(target_smry_A)[2]), collapse=''),
...
```{r, results='asis', echo=FALSE, Target_Sum_EOC}
	...
	# rgroups <- target_smry_B[, list(length(`Achievement Level`)), by=`Target Content Area`]$V1
  ...
  cat(dualTable(as.matrix(target_smry_B), title="", align=paste(rep('r', dim(target_smry_B)[2]), collapse=''),
  							n.cgroup=c(1, 1, dim(target_smry_B)[2]-2), cgroup=c("", "", "Growth Targets"), n.rgroup=rgroups,
...
```{r, results="asis", echo=FALSE, EOC_BoxPlots}
		placeFigure(
			...
			caption = "Language Arts related EOC 2019 growth target box plots by current achievement level.")

		placeFigure(
			files = "../img/Tech_Reports/2018/Box_Plots-3.png", page.break= TRUE,
			caption = "Mathematics related EOC 2019 growth target box plots by current achievement level.")
```


##  Growth Targets and Current Achievement

Examining the relationship between growth targets and current achievement also provides valuable insight into the targets and what they can tell us about expectations for student growth and proficiency attainment in the next year.  Correlation tables and scatter plots are provided in the subsequent sections to describe the relationship between Georgia's one-year targets and 2018 scale scores.  The correlation tables show negative relationships across the board, suggesting that lower achieving students have higher growth targets and vice versa.  This relationship is as expected - the further below an achievement level a student starts, the higher their target will be.  

Scatter plots show that this usually is not a simple linear relationship the Pearson correlation describes, with target ceilings and floors at the growth percentile and scale score extremes.  The scatter plots also help to show how the targets for the same current scores change relative to the three performance level target cuts.  The points in each scatter plot are color coded by current achievement level, and also contain contour lines that show the target distribution density more clearly.   
...
```{r, results='asis', echo=FALSE, Target_Cor_EOG}
  ...
  cat(dualTable(as.matrix(tmp_EOG_smry), align=paste(rep('r', dim(tmp_EOG_smry)[2]), collapse=''), caption = tmp.cap))
```
...
```{r, echo=FALSE, include=FALSE, cache=TRUE, Scatter_Plot2, fig.height=3.75, fig.width=5.5}
	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			...
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") + guides(col=guide_legend("Achvmnt Levs:"))
	...
	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			...
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") + guides(col=guide_legend("Achvmnt Levs:")) # + guides(col=guide_legend(nrow=2, "Grade 9 Lit Achievement Level"))
...
```{r, results='asis', echo=FALSE, Target_Cor_EOC}
  ...
  cat(dualTable(as.matrix(tmp_EOC_smry), align=paste(rep('r', dim(tmp_EOC_smry)[2]), collapse=''), caption = tmp.cap))
```
