...
2.  For students who participate in two main administrations in 2018 (i.e., students who failed and retook an EOC course in the same year), the first attempt is used as a prior to produce an SGP for the final attempt.  In subsequent years, their final attempt may be used as a prior for other EOC analyses.
...
In this step the data from step 1a is read into [`R`](http://www.r-project.org/) and modified slightly. The result is a `.Rdata` file containing data in the format suitable for analysis with the [`SGP` package](https://github.com/CenterForAssessment/SGP).  This data is combined with a subset of prior years' Milestones data necessary to complete the 2018 analyses and is stored in a new [`SGP` class object](http://cran.r-project.org/web/packages/SGP/SGP.pdf).  With an appropriate longitudinal data set prepared, we move to the calculation of student-level SGPs.
...
Due to differences in the time-frames in which the EOG and EOC were validated and made available, EOG and EOC SGPs were calculated at seperate times.  Cohort-referenced (uncorrected) and SIMEX corrected SGPs were calculated concurrently for each individual analysis.^[In addition to these quantities, baseline-referenced SGPs (uncorrected and SIMEX corrected) had been calculated in previous years and used as the "official" SGP quantity, but this method of SGP calculation will be unavailable until at least 3 years of Milestones assessment data are available for all analyses to form the baseline cohorts.]  We use the [`updateSGP`](https://www.rdocumentation.org/packages/SGP/versions/1.8-0.0/topics/updateSGP) function to ***a)*** do the final preparation and addition of the 2018 cleaned and formatted data to the new `SGP` class object ([`prepareSGP`](https://www.rdocumentation.org/packages/SGP/versions/1.8-0.0/topics/prepareSGP) step) and ***b)*** calculate SGP estimates ([`analyzeSGP`](https://www.rdocumentation.org/packages/SGP/versions/1.8-0.0/topics/analyzeSGP) step).  
...
```{r, results="asis", echo=FALSE, ELA_Bubble}
		placeFigure(
			...
			caption = "School-level Bubble Plots for Georgia: ELA, 2017-2018.")
```
...
```{r, results="asis", echo=FALSE, Math_Bubble}
		placeFigure(
			...
			caption = "School-level Bubble Plots for Georgia: Mathematics, 2017-2018.")
```
