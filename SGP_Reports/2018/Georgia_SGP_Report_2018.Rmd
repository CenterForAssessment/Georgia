...
```{r, echo=FALSE, include=FALSE}
  ...
  knitr::opts_chunk$set(cache.path = "_cache/GA_SGP_2018", fig.path="../img/Tech_Reports/2018/")
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_EOG <- table(Georgia_SGP@Data[YEAR=='2018' & CONTENT_AREA %in% GL_subjects & VALID_CASE=='VALID_CASE', GRADE, CONTENT_AREA])
```
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_EOC <- table(Georgia_SGP@Data[YEAR=='2018' & CONTENT_AREA %in% EOC_subjects & VALID_CASE=='VALID_CASE', GRADE, CONTENT_AREA])
```
...
Student growth projections were computed once both EOG and EOC student growth analyses had been completed.  The `analyzeSGP` function is also used for the growth projection/target computations.
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, GOFplots}
  ...
  dat <- Georgia_SGP@Data[SGP_NORM_GROUP %in% "2016/ELA_6; 2017/ELA_7; 2018/ELA_8", 
	...
	gofSGP(dat, state="GA", years='2018', content_areas="ELA", use.sgp="SGP_SIMEX", output.format="PNG")   #   Run for TAC presentation
  ...
  dat <- Georgia_SGP@Data[SGP_NORM_GROUP %in% "2018/GEOMETRY_EOCT; 2018/GEOMETRY_EOCT",
	...
	gofSGP(dat, state="GA", years='2018', content_areas='GEOMETRY', use.sgp="SGP_SIMEX", output.format="PNG")   #   Run for TAC presentation
...
```{r, results='asis', echo=FALSE, Student_EOG}
  ...
  cat(dualTable(as.matrix(gl_tmp_tbl), align=paste(rep('r', dim(gl_tmp_tbl)[2]), collapse=''), n.rgroup=c(5,5), caption = tmp.cap))
...
```{r, results='asis', echo=FALSE, Student_EOCT_1}
	...
	eoct_tmp_tbl$CONTENT_AREA <- sapply(eoct_tmp_tbl$CONTENT_AREA, capwords, USE.NAMES=FALSE)
	rgroups <- eoct_tmp_tbl[, list(length(Most_Recent_Prior)), by=CONTENT_AREA]$V1
	...
	cat(dualTable(as.matrix(eoct_tmp_tbl), align=paste(rep('r', dim(eoct_tmp_tbl)[2]), collapse=''), n.rgroup=rgroups, caption = tmp.cap))
...
```{r, results='asis', echo=FALSE, School_EOG_Correlations}
	...
	cat(dualTable(as.matrix(tmp_tbl), align=c('r', 'r', rep('c', dim(tmp_tbl)[2]-2)), n.rgroup=c(3,3), caption  = tmp.cap))
...
```{r, results='asis', echo=FALSE, School_Grade_EOG_Correlations}
	...
	cat(dualTable(as.matrix(gl_cor_tbl), align=c('r', 'r', rep('c', dim(gl_cor_tbl)[2]-2)), n.rgroup=c(5,5), caption = tmp.cap))
...
```{r, results='asis', echo=FALSE, School_EOC_Cor}
	...
	cat(dualTable(as.matrix(tmp_tbl), align=c('r', 'r', rep('c', dim(tmp_tbl)[2]-2)), n.rgroup=rep(3, length(EOC_subjects)), caption = tmp.cap))
...
Targets are initially established in the prior academic year, so that students' growth towards specific targets in the current year are considered relative to growth norms from the previous years' cohort.  The growth target analyses done in 2018 will be used in the [Georgia visualization tool](http://gastudentgrowth.gadoe.org/) to provide stakeholders with prospective information about student growth needs in the upcoming year.  These targets will also be used to compare with 2019 SGPs to determine growth adequacy.  The following sections provide descriptive statistics of the growth target results.
...
The following tables and figures summarize the distributions of the 2019 Developing, Proficient and Distinguished Growth Targets for all students in 2018.  The summaries are disaggregated by the current achievement level to demonstrate how students' academic starting points impacts their one-year growth targets.  As would be expected students in lower achievement levels need to demonstrate more growth than students with higher achievement to reach the same proficiency levels.  These summary tables and visualizations help us better understand the extent to which some groups must grow in the next year, the likelihood of those goals being attained, and the variability of those growth targets within achievement level groups.

```{r, cache=TRUE, echo=FALSE, include=FALSE, Target_Sum}
	target_smry <- Georgia_SGP@Data[,list( # [YEAR=='2018' & CONTENT_AREA %in% GL_subjects]
				MEDIAN_Developing=median(as.numeric(DEVELOPING_SGP_TARGET_YEAR_1_CURRENT), na.rm=TRUE),
				MEAN_Developing=format(round(mean(DEVELOPING_SGP_TARGET_YEAR_1_CURRENT, na.rm=TRUE), 1), nsmall = 1),
				MEDIAN_Proficient=median(as.numeric(PROFICIENT_SGP_TARGET_YEAR_1_CURRENT), na.rm=TRUE),
				MEAN_Proficient=format(round(mean(PROFICIENT_SGP_TARGET_YEAR_1_CURRENT, na.rm=TRUE), 1), nsmall = 1),
				MEDIAN_Distinguished=median(as.numeric(DISTINGUISHED_SGP_TARGET_YEAR_1_CURRENT), na.rm=TRUE),
				MEAN_Distinguished=format(round(mean(DISTINGUISHED_SGP_TARGET_YEAR_1_CURRENT, na.rm=TRUE), 1), nsmall = 1)),
			by=c('CONTENT_AREA', 'ACHIEVEMENT_LEVEL', 'SGP_PROJECTION_GROUP_CURRENT')][!is.na(ACHIEVEMENT_LEVEL)][!is.na(SGP_PROJECTION_GROUP_CURRENT)]

	target_smry_long <- merge(
		melt(target_smry, id = c('CONTENT_AREA', 'ACHIEVEMENT_LEVEL', 'SGP_PROJECTION_GROUP_CURRENT'), measure.vars = c("MEDIAN_Developing", "MEDIAN_Proficient", "MEDIAN_Distinguished"), variable.name = "Target", value.name = "Median")[, Target := gsub("MEDIAN_", "", Target)],
		melt(target_smry, id = c('CONTENT_AREA', 'ACHIEVEMENT_LEVEL', 'SGP_PROJECTION_GROUP_CURRENT'), measure.vars = c("MEAN_Developing", "MEAN_Proficient", "MEAN_Distinguished"), variable.name = "Target", value.name = "Mean")[, Target := gsub("MEAN_", "", Target)], by=c("CONTENT_AREA", "ACHIEVEMENT_LEVEL", 'SGP_PROJECTION_GROUP_CURRENT', "Target"))

	target_smry_long[, ACHIEVEMENT_LEVEL := factor(ACHIEVEMENT_LEVEL, levels=c("Beginning Learner", "Developing Learner", "Proficient Learner", "Distinguished Learner"))]
	target_smry_long[, Target := factor(Target, levels=c("Developing", "Proficient", "Distinguished"))]
	target_smry_long[, SGP_PROJECTION_GROUP_CURRENT := gsub("MATH_ALG_I", "MATHEMATICS", SGP_PROJECTION_GROUP_CURRENT)]

	setkey(target_smry_long)
```

```{r, echo=FALSE, include=FALSE, cache=TRUE, Box_Plots, fig.height=8, fig.width=6}
	dir.create("../img/Tech_Reports/2018/", recursive = TRUE, showWarnings = FALSE)

  tmp.data <- Georgia_SGP@Data[, list(SCALE_SCORE, SGP_PROJECTION_GROUP_CURRENT, DEVELOPING_SGP_TARGET_YEAR_1_CURRENT, PROFICIENT_SGP_TARGET_YEAR_1_CURRENT, DISTINGUISHED_SGP_TARGET_YEAR_1_CURRENT, ACHIEVEMENT_LEVEL)] # SGP_PROJECTION_GROUP_CURRENT %in% c("ELA", "MATH_ALG_I")
 	tmp.data[, SGP_PROJECTION_GROUP_CURRENT := gsub("MATH_ALG_I", "MATHEMATICS", SGP_PROJECTION_GROUP_CURRENT)]
	tmp.data[, SGP_PROJECTION_GROUP_CURRENT := factor(SGP_PROJECTION_GROUP_CURRENT, levels=subject_order, labels=sapply(subject_order, capwords, USE.NAMES = FALSE))]
 	tmp.data[, ACHIEVEMENT_LEVEL := gsub(" Learner", "", ACHIEVEMENT_LEVEL)]
 	tmp.data[, ACHIEVEMENT_LEVEL := factor(ACHIEVEMENT_LEVEL, levels=c("Beginning", "Developing", "Proficient", "Distinguished"))]
	tmp.data <- melt(tmp.data, id = c('SCALE_SCORE', 'ACHIEVEMENT_LEVEL', 'SGP_PROJECTION_GROUP_CURRENT'), measure.vars = c("DEVELOPING_SGP_TARGET_YEAR_1_CURRENT", "PROFICIENT_SGP_TARGET_YEAR_1_CURRENT", "DISTINGUISHED_SGP_TARGET_YEAR_1_CURRENT"), variable.name = "Target_Level", value.name = "Target")
	tmp.data[, Target_Level := gsub("_SGP_TARGET_YEAR_1_CURRENT", "", Target_Level)]
 	tmp.data[, Target_Level := factor(Target_Level, levels=c("DEVELOPING", "PROFICIENT", "DISTINGUISHED"), labels=c("Developing Target", "Proficient Target", "Distinguished Target"))]
	setnames(tmp.data, c("ACHIEVEMENT_LEVEL", "SGP_PROJECTION_GROUP_CURRENT", "Target_Level"), c("Achievement Level", "Target Content Area", "Target Level"))

	ggplot(data=tmp.data[`Target Content Area` %in% c("ELA", "Mathematics"), ], aes(x=`Target Content Area`, y=Target, col=`Target Content Area`)) + 
				geom_boxplot() + labs(x="", y="") + scale_color_brewer(palette="Set1") + 
				theme(axis.text.x = element_blank(), legend.position = "bottom") + facet_grid(`Achievement Level` ~ `Target Level`)

	ggplot(data=tmp.data[`Target Content Area` %in% c("Grade 9 Lit", "American Lit"), ], aes(x=`Target Content Area`, y=Target, col=`Target Content Area`)) +
				geom_boxplot() + labs(x="", y="") + scale_color_brewer(palette="Dark2") + 
				theme(axis.text.x = element_blank(), legend.position = "bottom") + facet_grid(`Achievement Level` ~ `Target Level`)

	ggplot(data=tmp.data[`Target Content Area` %in% c("Algebra I", "Geometry", "Coordinate Algebra", "Analytic Geometry"), ], aes(x=`Target Content Area`, y=Target, col=`Target Content Area`)) +
				geom_boxplot() + labs(x="", y="") + scale_color_brewer(palette="Paired") + 
				theme(axis.text.x = element_blank(), legend.position = "bottom", legend.title = element_blank()) + 
				facet_grid(`Achievement Level` ~ `Target Level`) + guides(col=guide_legend(nrow=2))
```

###  End of Grade Growth Targets

The EOG summaries include targets that project from one grade level to the next.  This includes ELA and Mathematics targets for grades 4 through 8 in 2019 (i.e. 2018 3<sup>rd</sup> to 7<sup>th</sup> graders).

```{r, results='asis', echo=FALSE, Target_Sum_EOG}
	target_smry_A <- data.frame()
	for (ca in GL_subjects) {
		tmp.ca <- ca
		for (achlev in c("Beginning Learner", "Developing Learner", "Proficient Learner", "Distinguished Learner")) {
			tmp_data <- target_smry_long[SGP_PROJECTION_GROUP_CURRENT==ca & ACHIEVEMENT_LEVEL==achlev,]
			tmp_EOG_smry <- paste(t(as.matrix(tmp_data[, list(Median)])), " (", t(as.matrix(tmp_data[, list(Mean)])), ")", sep="")
			tmp_EOG_smry <- data.frame(matrix(c(capwords(tmp.ca), achlev, tmp_EOG_smry), 1, length(tmp_EOG_smry)+2))
			names(tmp_EOG_smry) <- c("Target Content Area", "Current Achievement Level", t(as.matrix(tmp_data[, list(Target)])))
			target_smry_A <- rbindlist(list(target_smry_A, tmp_EOG_smry))
			if (achlev == "Beginning Learner") tmp.ca <- ""
		}
	}
	target_smry_A[is.na(target_smry_A)] <- ""
	
  cat(dualTable(as.matrix(target_smry_A), title="", align=paste(rep('r', dim(target_smry_A)[2]), collapse=''),
  							n.cgroup=c(1, 1, dim(target_smry_A)[2]-2), cgroup=c("", "", "Growth Target Levels"), n.rgroup=c(4,4),
  							caption='Median (Mean) 2019 EOG Growth Targets by Target Content Area and Current Achievement Level'))
```

```{r, results="asis", echo=FALSE, EOG_BoxPlots}
		placeFigure(
			files = "../img/Tech_Reports/2018/Box_Plots-1.png", page.break= TRUE,
			caption = "2019 EOG growth target box plots by current achievement level.")
```

###  End of Course Growth Targets

The EOC summaries include targets that project from one content area to the next related subject that represents the most typical course progression in Georgia.  For example, 8<sup>th</sup> grade students typically move from grade level ELA and Mathematics to 9<sup>th</sup> Grade Literature and Algebra I respectively, and then subsequently from these content areas to American Literature and Geometry.  Some less typical, but important, course progression targets are also calculated.  For example, each 8<sup>th</sup> grade student has both a Grade 8 Math to Algebra I and Grade 8 Math to Coordinate Algebra target calculated in 2018.  The target used for each student with depend on the high school math course actually taken in 2019.


```{r, results='asis', echo=FALSE, Target_Sum_EOC}
	target_smry_B <- data.frame()
	for (ca in EOC_subjects) {
		tmp.ca <- ca
		for (achlev in c("Beginning Learner", "Developing Learner", "Proficient Learner", "Distinguished Learner")) {
			tmp_data <- target_smry_long[SGP_PROJECTION_GROUP_CURRENT==ca & ACHIEVEMENT_LEVEL==achlev,]
			tmp_EOG_smry <- paste(t(as.matrix(tmp_data[, list(Median)])), " (", t(as.matrix(tmp_data[, list(Mean)])), ")", sep="")
			tmp_EOG_smry <- data.frame(matrix(c(capwords(tmp.ca), achlev, tmp_EOG_smry), 1, length(tmp_EOG_smry)+2))
			names(tmp_EOG_smry) <- c("Target Content Area", "Current Achievement Level", t(as.matrix(tmp_data[, list(Target)])))
			target_smry_B <- rbindlist(list(target_smry_B, tmp_EOG_smry))
			if (achlev == "Beginning Learner") tmp.ca <- ""
		}
	}
	target_smry_B[is.na(target_smry_B)] <- ""
	
	rgroups <- rep(4, length(EOC_subjects))

  cat(dualTable(as.matrix(target_smry_B), title="", align=paste(rep('r', dim(target_smry_B)[2]), collapse=''),
  							n.cgroup=c(1, 1, dim(target_smry_B)[2]-2), cgroup=c("", "", "Growth Target Levels"), n.rgroup=rgroups,
  							caption='2019 EOC Median (Mean) Growth Targets by Subject and Current Achievement.'))
```

```{r, results="asis", echo=FALSE, EOC_BoxPlots}
		placeFigure(
			files = "../img/Tech_Reports/2018/Box_Plots-2.png", page.break= TRUE,
			caption = "English Language Arts EOC 2019 growth target box plots by current achievement level.")

		placeFigure(
			files = "../img/Tech_Reports/2018/Box_Plots-3.png", page.break= TRUE,
			caption = "Mathematics EOC 2019 growth target box plots by current achievement level.")
```


##  Growth Targets and Current Achievement

Examining the relationship between growth targets and current achievement also provides valuable insight into the targets and what they can tell us about expectations for student growth and proficiency attainment on the next assessment.  Correlation tables and scatter plots are provided in the subsequent sections to describe the relationship between Georgia's growth targets and 2018 scale scores.  The correlation tables show negative relationships across the board, suggesting that lower achieving students have higher growth targets and vice versa.  This relationship is as expected - the further below an achievement level a student starts, the higher their target will be.

Scatter plots show that this usually is not a simple linear relationship as the Pearson correlation describes due to the ceiling and floor of the growth targets at the lower and higher end of the current scale scores distributions.  The scatter plots also help to show how the targets for the same current scores change relative to the three performance level target cuts.  The points in each scatter plot are color coded by current achievement level, and also contain contour lines that show the target distribution density more clearly.

```{r, cache=TRUE, echo=FALSE, include=FALSE, Target_Cor}
	target_cor <- Georgia_SGP@Data[,list( # [YEAR=='2018' & CONTENT_AREA %in% GL_subjects]
				Developing=round(cor(SCALE_SCORE, DEVELOPING_SGP_TARGET_YEAR_1_CURRENT, use='pairwise.complete'), 2),
				# Prior_Developing=round(cor(SCALE_SCORE_PRIOR_STANDARDIZED, DEVELOPING_SGP_TARGET_YEAR_1_CURRENT, use='pairwise.complete'), 2),
				Proficient=round(cor(SCALE_SCORE, PROFICIENT_SGP_TARGET_YEAR_1_CURRENT, use='pairwise.complete'), 2),
				Distinguished=round(cor(SCALE_SCORE, DISTINGUISHED_SGP_TARGET_YEAR_1_CURRENT, use='pairwise.complete'), 2), N = .N),
			by=c('CONTENT_AREA', 'SGP_PROJECTION_GROUP_CURRENT', 'GRADE')][!is.na(SGP_PROJECTION_GROUP_CURRENT)]

	target_cor[, SGP_PROJECTION_GROUP_CURRENT := gsub("MATH_ALG_I", "MATHEMATICS", SGP_PROJECTION_GROUP_CURRENT)]
...
```{r, results='asis', echo=FALSE, Target_Cor_EOG}
	tmp_EOG_smry <- target_cor[SGP_PROJECTION_GROUP_CURRENT %in% GL_subjects]
	tmp_EOG_smry[, GRADE := as.numeric(GRADE)+1]
	tmp_EOG_smry[, CONTENT_AREA := NULL]
	tmp_EOG_smry[, SGP_PROJECTION_GROUP_CURRENT := sapply(SGP_PROJECTION_GROUP_CURRENT, SGP::capwords)]
	tmp_EOG_smry[, N := prettyNum(N, preserve.width = "individual", big.mark=',')]
	setkey(tmp_EOG_smry, SGP_PROJECTION_GROUP_CURRENT, GRADE)
	# tmp_EOG_smry <- tmp_EOG_smry[][order(match(tmp_EOG_smry$SGP_PROJECTION_GROUP_CURRENT, GL_subjects))]
	tmp_EOG_smry[duplicated(SGP_PROJECTION_GROUP_CURRENT), SGP_PROJECTION_GROUP_CURRENT := ""]
	setnames(tmp_EOG_smry, c("SGP_PROJECTION_GROUP_CURRENT", "GRADE"), c("Target Content Area", "Target Grade"))

	tmp.cap <- "EOG Student Level Correlations between 2019 Growth Target and Current Scale Score."

  cat(dualTable(as.matrix(tmp_EOG_smry), align=paste(rep('r', dim(tmp_EOG_smry)[2]), collapse=''),
  							n.cgroup=c(2, dim(tmp_EOG_smry)[2]-3, 1), cgroup=c("", "Growth Target Levels", ""), n.rgroup=c(5,5), caption = tmp.cap))
```

```{r, echo=FALSE, include=FALSE, cache=TRUE, Scatter_Plot1, fig.height=3.25, fig.width=5.5}
	levels(tmp.data[["Target Level"]]) <- c("Developing", "Proficient", "Distinguished")
	tmp.points <- unique(tmp.data, by=c("SCALE_SCORE", "Target", "Target Level", "Target Content Area"))[!is.na(Target)]

	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) + # NULL data to use different data sets below
			theme_bw() + geom_point(data=tmp.points[`Target Content Area` == "ELA"], size=0.5) + # , alpha=0.65
			stat_density2d(data=tmp.data[`Target Content Area` == "ELA"], geom="density2d", colour="red", contour = TRUE) + #, size=1
			labs(y = "2019 Target", x = "Current Grade Level ELA Scale Score") + theme(axis.title=element_text(size=10, face="bold"), legend.position="none", legend.title = element_blank()) + 
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") # + guides(col=guide_legend(nrow=2, "Current ELA Achievement Level"))

	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			theme_bw() + geom_point(data=tmp.points[`Target Content Area` == "Grade 9 Lit"], size=0.5) +
			stat_density2d(data=tmp.data[`Target Content Area` == "Grade 9 Lit"], geom="density2d", colour="red", contour = TRUE) +
			labs(y = "2019 Target", x = "Grade 8 ELA Scale Score") + theme(axis.title=element_text(size=10, face="bold"), legend.position="none", legend.title = element_blank()) + 
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") # + guides(col=guide_legend(nrow=2, "Grade 8 ELA Achievement Level"))

	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			theme_bw() + geom_point(data=tmp.points[`Target Content Area` == "Algebra I"], size=0.5) +
			stat_density2d(data=tmp.data[`Target Content Area` == "Algebra I"], geom="density2d", colour="red", contour = TRUE) +
			labs(y = "2019 Target", x = "Grade 8 Math Scale Score") + theme(axis.title=element_text(size=10, face="bold"), legend.position="none", legend.title = element_blank()) + 
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") # + guides(col=guide_legend(nrow=2, "Grade 8 Math Achievement Level"))

	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			theme_bw() + geom_point(data=tmp.points[`Target Content Area` == "Coordinate Algebra"], size=0.5) +
			stat_density2d(data=tmp.data[`Target Content Area` == "Coordinate Algebra"], geom="density2d", colour="red", contour = TRUE) +
			labs(y = "2019 Target", x = "Grade 8 Math Scale Score") + theme(axis.title=element_text(size=10, face="bold"), legend.position="none", legend.title = element_blank()) + 
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") # + guides(col=guide_legend(nrow=2, "Grade 8 Math Achievement Level"))

	# ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
	# 		theme_bw() + geom_point(data=tmp.points[`Target Content Area` %in% c("Coordinate Algebra", "Analytic Geometry")], size=0.5) +
	# 		stat_density2d(data=tmp.data[`Target Content Area` %in% c("Coordinate Algebra", "Analytic Geometry")], geom="density2d", colour="red", contour = TRUE) +
	# 		labs(y = "2019 Target", x = "Current Scale Score") + theme(axis.title=element_text(size=10, face="bold"), legend.position="bottom") + 
	# 		facet_grid(`Target Content Area` ~ `Target Level`) + scale_color_brewer(palette="Paired") + guides(col=guide_legend(nrow=2, "Current Achievement Level"))
```

```{r, echo=FALSE, include=FALSE, cache=TRUE, Scatter_Plot2, fig.height=3.75, fig.width=5.5}
	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			theme_bw() + geom_point(data=tmp.data[`Target Content Area` == "Mathematics"], size=0.5) +
			stat_density2d(data=tmp.data[`Target Content Area` == "Mathematics"], geom="density2d", colour="red", contour = TRUE, size=0.5) +
			labs(y = "2019 Target", x = "Current Grade Level Math Scale Score") + theme(axis.title=element_text(size=6, face="bold"), legend.position="bottom") + 
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") + guides(col=guide_legend("Achievement Levels:", size = 6, title.theme = element_text(size = 8, face = "bold")))

	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			theme_bw() + geom_point(data=tmp.points[`Target Content Area` == "American Lit"], size=0.5) +
			stat_density2d(data=tmp.data[`Target Content Area` == "American Lit"], geom="density2d", colour="red", contour = TRUE) +
			labs(y = "2019 Target", x = "Grade 9 Lit Scale Score") + theme(axis.title=element_text(size=6, face="bold"), legend.position="bottom") + 
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") + guides(col=guide_legend("Achievement Levels:", size = 6, title.theme = element_text(size = 8, face = "bold"))) # + guides(col=guide_legend(nrow=2, "Grade 9 Lit Achievement Level"))

	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			theme_bw() + geom_point(data=tmp.points[`Target Content Area` == "Geometry"], size=0.5) +
			stat_density2d(data=tmp.data[`Target Content Area` == "Geometry"], geom="density2d", colour="red", contour = TRUE) +
			labs(y = "2019 Target", x = "Algebra I Scale Score")  + theme(axis.title=element_text(size=6, face="bold"), legend.position="bottom") + 
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") + guides(col=guide_legend("Achievement Levels:", size = 6, title.theme = element_text(size = 8, face = "bold")))

	ggplot(NULL, aes(x=SCALE_SCORE, y=Target, col=`Achievement Level`)) +
			theme_bw() + geom_point(data=tmp.points[`Target Content Area` == "Analytic Geometry"], size=0.5) +
			stat_density2d(data=tmp.data[`Target Content Area` == "Analytic Geometry"], geom="density2d", colour="red", contour = TRUE) +
			labs(y = "2019 Target", x = "Coordinate Algebra Scale Score") + theme(axis.title=element_text(size=6, face="bold"), legend.position="bottom") + 
			facet_grid(`Target Level`~.) + scale_color_brewer(palette="Paired") + guides(col=guide_legend("Achievement Levels:", size = 6, title.theme = element_text(size = 8, face = "bold")))
```

```{r, results="asis", echo=FALSE, EOG_ScatterPlots}
		placeFigure(
			files = "../img/Tech_Reports/2018/Scatter_Plot1-1.png", pdf.width = 0.95,
			caption = "ELA 2019 growth target box plots by current scale score.")

		placeFigure(
			files = "../img/Tech_Reports/2018/Scatter_Plot2-1.png", pdf.width = 0.95, page.break= TRUE,
			caption = "Mathematics 2019 growth target box plots by current scale score.")
```

###   End of Course Test Subjects

```{r, results='asis', echo=FALSE, Target_Cor_EOC}
	tmp_EOC_smry <- target_cor[SGP_PROJECTION_GROUP_CURRENT %in% EOC_subjects]
	tmp_EOC_smry[, GRADE := NULL]
	tmp_EOC_smry[, CONTENT_AREA := sapply(CONTENT_AREA, SGP::capwords)]
	tmp_EOC_smry <- tmp_EOC_smry[][order(match(tmp_EOC_smry$SGP_PROJECTION_GROUP_CURRENT, EOC_subjects))]
	tmp_EOC_smry[, SGP_PROJECTION_GROUP_CURRENT := sapply(SGP_PROJECTION_GROUP_CURRENT, SGP::capwords)]
	tmp_EOC_smry[, N := prettyNum(N, preserve.width = "individual", big.mark=',')]
	# setkey(tmp_EOC_smry, CONTENT_AREA, SGP_PROJECTION_GROUP_CURRENT)
	tmp_EOC_smry[duplicated(SGP_PROJECTION_GROUP_CURRENT), SGP_PROJECTION_GROUP_CURRENT := ""]
	setnames(tmp_EOC_smry, c("CONTENT_AREA", "SGP_PROJECTION_GROUP_CURRENT"), c("Content Area", "Target Content Area"))

	tmp.cap <- "EOC Student Level Correlations between 2019 Growth Target and Current Scale Score."

  cat(dualTable(as.matrix(tmp_EOC_smry), align=paste(rep('r', dim(tmp_EOC_smry)[2]), collapse=''),
  							n.cgroup=c(2, dim(tmp_EOC_smry)[2]-3, 1), cgroup=c("", "Growth Target Levels", ""), caption = tmp.cap))
```
...
```{r, results="asis", echo=FALSE, EOC_ScatterPlots}
		placeFigure(
			files = "../img/Tech_Reports/2018/Scatter_Plot1-2.png", pdf.width = 0.95,
			caption = "Grade 9 Lit 2019 growth targets by current scale score.")

		placeFigure(
			files = "../img/Tech_Reports/2018/Scatter_Plot2-2.png", pdf.width = 0.95, page.break= TRUE,
			caption = "American Lit 2019 growth targets by current scale score.")
...
######
		...
		placeFigure(
			files = "../img/Tech_Reports/2018/Scatter_Plot1-3.png", pdf.width = 0.95,
			caption = "2019 Algebra I growth targets by current scale score.")

		placeFigure(
			files = "../img/Tech_Reports/2018/Scatter_Plot2-3.png", pdf.width = 0.95,
			caption = "2019 Geometry growth targets by current scale score.", page.break= TRUE)
...
######
		...
		placeFigure(
			files = "../img/Tech_Reports/2018/Scatter_Plot1-4.png", pdf.width = 0.95,
			caption = "Coordinate Algebra 2019 growth targets by current scale score.")

		placeFigure(
			files = "../img/Tech_Reports/2018/Scatter_Plot2-4.png", pdf.width = 0.95, page.break= TRUE,
			caption = "Analytic Geometry 2019 growth targets by current scale score.")
