...
date: April 2015
abstract:  "DRAFT REPORT - DO NOT CITE! <br></br> This report provides details about the Georgia Student 
  Growth Model methodology and presents a descriptive analysis of the 2014 SGP calculation process and results."
...
```{r, echo=FALSE, include=FALSE}
  ...
  knitr::opts_chunk$set(cache.path = "_cache/GA_SGP_2014")
...
#   options(table_counter_str = "<strong>Table %s:</strong> ")
  options(tbl_n=0)
  tblNum <- function() {return(getOption("tbl_n"))}
  tblNumNext <- function() {return(getOption("tbl_n")+1)}
  tblNumIncrement <- function() {options("tbl_n" = getOption("tbl_n")+1); return(getOption("tbl_n"))}
  tblCap <- function(caption.text) {
    tblNumIncrement()
    return(paste("**Table ", getOption("tbl_n"), ":** ", caption.text, sep=""))
  }
  tblCap_NO_Increment <- function(caption.text) {
    return(paste("**Table ", getOption("tbl_n"), ":** ", caption.text, sep=""))
  }
  tblCapNULL <- function() {
    options("tbl_n" = getOption("tbl_n")+1)
    return(NULL)
  }
  ...
  dualTable <- function(my_tbl, where="!htbp", caption, css.class = "gmisc_table breakboth", ...) {
    tmp_caption <- tblCap(caption)
    cat("\n<!-- HTML_Start -->\n")
    ## Use suppressWarnings() with htmlTable for incorrect warning asking rnames to be specified... 
    html.text <- suppressWarnings(cat(htmlTable(my_tbl, title="", caption=tmp_caption, css.class=css.class,...)))
    cat("\n<!-- LaTeX_Start\n")
    latex.text<- print(latex(my_tbl, file="", where=where, caption=tmp_caption, ...))
    cat("\nLaTeX_End -->\n")
  ...
  subject_order <- c("READING", "ELA","SOCIAL_STUDIES", "SCIENCE",  "MATHEMATICS", 
	...
	###  Set working directory to Georgia Github Repo for Documentation
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_crct <- table(Georgia_SGP@Data[YEAR=='2014' & CONTENT_AREA %in% CRCT_subjects & VALID_CASE=='VALID_CASE']$CONTENT_AREA, Georgia_SGP@Data[YEAR=='2014' & CONTENT_AREA %in% CRCT_subjects & VALID_CASE=='VALID_CASE']$GRADE)
```
```{r, results='asis', echo=FALSE, N_tableCRCT}
	n_tbl_crctB <- n_tbl_crct[match(CRCT_subjects, row.names(n_tbl_crct)) ,]
	n_tbl_crctB <- cbind('Content Area'=sapply(CRCT_subjects, capwords, special.words=c("ELA", "US")), n_tbl_crctB)
	n_tbl_crctB <- prettyNum(n_tbl_crctB, preserve.width = "individual",big.mark=',')
	n_tbl_crctB[which(n_tbl_crctB==0)] <- ''
# 	n_tbl_crctB <- n_tbl_crctB[, match(c('Content Area', sort(as.numeric(colnames(n_tbl_crctB)[-1]))), colnames(n_tbl_crctB))]
# 	n_tbl_crctB <- n_tbl_crctB[, !is.na(colnames(n_tbl_crctB))]
	row.names(n_tbl_crctB) <- NULL

  cat(dualTable(as.matrix(n_tbl_crctB), align=paste(rep('r', dim(n_tbl_crctB)[2]), collapse=''), 
		n.cgroup=c(1, dim(n_tbl_crctB)[2]-1), cgroup=c("", "Grades"),
		caption='Number of Valid CRCT Student Records by Grade and Subject for 2014'))

#   cat("\n<!-- HTML_Start -->\n")
#   options("table_counter"=1)
# 	cat(htmlTable(as.matrix(n_tbl_crctB), file="", title="", align=paste(rep('r', dim(n_tbl_crctB)[2]), collapse=''), 
# 		n.cgroup=c(1, dim(n_tbl_crctB)[2]-1), cgroup=c("", "Grades"),
# 		caption='Number of Valid CRCT Student Records by Grade and Subject for 2014'))
# 
#   cat("\n<!-- LaTeX_Start\n")
#   print(latex(as.matrix(n_tbl_crctB), file="", title="", align=paste(rep('r', dim(n_tbl_crctB)[2]), collapse=''),
#   	n.cgroup=c(1, dim(n_tbl_crctB)[2]-1), cgroup=c("", "Grades"), compatibility="CSS",
# 		caption='Number of Valid CRCT Student Records by Grade and Subject for 2014'))
#   cat("\nLaTeX_End -->\n")

#  z <- ztable(n_tbl_crctB, include.rownames=FALSE, align=paste(rep('r', dim(n_tbl_crctB)[2]), collapse=''), 
#              caption='Number of Valid CRCT Student Records by Grade and Subject for 2014',  type="latex")
#  z <- addcgroup(z, cgroup=c("", "Grades"),n.cgroup=c(1, dim(n_tbl_crctB)[2]-1))						
#	closeAllConnections()
```
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_eoct <- table(Georgia_SGP@Data[YEAR=='2014' & CONTENT_AREA %in% EOCT_subjects & VALID_CASE=='VALID_CASE']$CONTENT_AREA, Georgia_SGP@Data[YEAR=='2014' & CONTENT_AREA %in% EOCT_subjects & VALID_CASE=='VALID_CASE']$GRADE)
```
```{r, results='asis', echo=FALSE, N_tableEOCT}
	n_tbl_eoctB <- n_tbl_eoct[match(EOCT_subjects, row.names(n_tbl_eoct)) ,]
	n_tbl_eoctB <- cbind('Content Area'=sapply(EOCT_subjects, capwords, special.words=c("ELA", "II", "US")), 'Valid Records' = n_tbl_eoctB)
	n_tbl_eoctB <- prettyNum(n_tbl_eoctB, preserve.width = "individual",big.mark=',')
	row.names(n_tbl_eoctB) <- NULL

  cat(dualTable(as.matrix(n_tbl_eoctB), align=paste(rep('r', dim(n_tbl_eoctB)[2]), collapse=''), 
		caption='Total Number of Valid EOCT Student Records by Subject for 2014'))

#   cat("\n<!-- HTML_Start -->\n")
# #   options("table_counter"=getOption("table_counter")-1)
#   cat(htmlTable(as.matrix(n_tbl_eoctB), file="", title="", align=paste(rep('r', dim(n_tbl_eoctB)[2]), collapse=''), 
# 		# n.cgroup=c(1, dim(n_tbl_eoctB)[2]-1), cgroup=c("", "Grades"),
# 		caption='Total Number of Valid EOCT Student Records by Subject for 2014'))
# 
#   cat("\n<!-- LaTeX_Start\n")
#   print(latex(as.matrix(n_tbl_eoctB), file="", title="", align=paste(rep('r', dim(n_tbl_eoctB)[2]), collapse=''),
# 		caption='Total Number of Valid EOCT Student Records by Subject for 2014'))
#   cat("\nLaTeX_End -->\n")
# 
#  z <- ztable(as.matrix(n_tbl_eoctB), align=paste(rep('r', dim(n_tbl_eoctB)[2]), collapse=''), 
#      caption='Total Number of Valid EOCT Student Records by Subject for 2014', type="latex")
#  z <- addcgroup(z, n.cgroup=c(1, dim(n_tbl_eoctB)[2]-1))      			
...
```{r, results='asis', echo=FALSE, cohortSmry}
	...
	chrt_eoct_smry_B <- chrt_eoct_smry_B[, CONTENT_AREA := sapply(CONTENT_AREA, capwords, special.words=c("II", "US"))] #  need to use <- assignment with := to avoid print out of DT...
	setnames(chrt_eoct_smry_B, "CONTENT_AREA", "Content Area")
	
  cat(dualTable(as.matrix(chrt_eoct_smry_B), align='rcc', 
		caption='EOCT Median and Mean <em>Cohort</em> Referenced Student Growth Percentile by Content Area for 2014'))

#   cat("\n<!-- HTML_Start -->\n")
# #   options("table_counter"=getOption("table_counter")-1)
#   cat(htmlTable(as.matrix(chrt_eoct_smry_B), file="", title="", align='rcc', 
# 		# n.cgroup=c(1, dim(chrt_eoct_smry_B)[2]-1), cgroup=c("", "Grades"),
# 		caption='EOCT Median and Mean <em>Cohort</em> Referenced Student Growth Percentile by Content Area for 2014'))
# 
#   cat("\n<!-- LaTeX_Start\n")
#   print(latex(as.matrix(chrt_eoct_smry_B), file="", title="", align='rcc', 
#   	caption='EOCT Median and Mean <em>Cohort</em> Referenced Student Growth Percentile by Content Area for 2014'))
#   cat("\nLaTeX_End -->\n")
    
#  z <- ztable(as.matrix(chrt_eoct_smry_B), align='rcc', 
#    caption='EOCT Median and Mean <em>Cohort</em> Referenced Student Growth Percentile by Content Area for 2014', type="latex")
...
```{r, results='asis', echo=FALSE, baseSumCRCT}
	...
	for (ca in CRCT_subjects){
		tmp_bsln_crct_smry <- paste(t(as.matrix(bsln_crct_smry[CONTENT_AREA==ca,][, list(MEDIAN)])), " (", t(as.matrix(bsln_crct_smry[CONTENT_AREA==ca,][, list(MEAN)])), ")", sep="")
		tmp_bsln_crct_smry <- data.frame(matrix(c(capwords(ca), tmp_bsln_crct_smry), 1, length(tmp_bsln_crct_smry)+1))
		names(tmp_bsln_crct_smry) <- c("Content Area", t(as.matrix(bsln_crct_smry[CONTENT_AREA==ca,][, list(GRADE)])))
		bsln_crct_smryB <- rbind.fill(bsln_crct_smryB, tmp_bsln_crct_smry)
	}
...
# 
#   cat("\n<!-- LaTeX_Start\n")
#   print(latex(as.matrix(bsln_crct_smryB), file="", title="", align=paste(rep('r', dim(bsln_crct_smryB)[2]), collapse=''), 
#     n.cgroup=c(1, dim(bsln_crct_smryB)[2]-1), cgroup=c("", "Grades"),
# 		caption='CRCT Median (Mean) <em>Baseline</em> Student Growth Percentile by Grade and Content Area for 2014'))
#   cat("\nLaTeX_End -->\n")
#     
#  z <- ztable(as.matrix(chrt_eoct_smry_B), align='rcc', 
#           caption='EOCT Median and Mean <em>Cohort</em> Referenced Student Growth Percentile by Content Area for 2014', type="latex")
#  z <- addcgroup(z, n.cgroup=c(1, dim(chrt_eoct_smry_B)[2]-1))    				
...
```{r, results='asis', echo=FALSE, bslnSmry}
	...
	bsln_eoct_smry_B <- bsln_eoct_smry_B[, CONTENT_AREA := sapply(CONTENT_AREA, capwords, special.words=c("II", "US"))]
...
# 
#   cat("\n<!-- LaTeX_Start\n")
#   print(latex(as.matrix(bsln_eoct_smry_B), file="", title="", align='rcc', 
#   	caption='EOCT Median and Mean <em>Baseline</em> Referenced Student Growth Percentile by Content Area for 2014'))
#   cat("\nLaTeX_End -->\n")
    
#  z <- ztable(as.matrix(bsln_eoct_smry_B), align='rcc', type="latex",
#         caption='EOCT Median and Mean <em>Baseline</em> Referenced Student Growth Percentile by Content Area for 2014')
```
...
```{r, results='asis', echo=FALSE, chrt_SIMEX_smry_B}
	...
	chrt_SIMEX_smry_B <- chrt_SIMEX_smry_B[, CONTENT_AREA := sapply(CONTENT_AREA, capwords, special.words=c("II", "US"))]
	setnames(chrt_SIMEX_smry_B, "CONTENT_AREA", "Content Area")

  cat(dualTable(as.matrix(chrt_SIMEX_smry_B), align='rcc', 
		caption='SIMEX Corrected EOCT Median and Mean <em>Cohort</em> Referenced Student Growth Percentile by Content Area for 2014'))

#   cat("\n<!-- HTML_Start -->\n")
# #   options("table_counter"=getOption("table_counter")-1)
#   cat(htmlTable(as.matrix(chrt_SIMEX_smry_B), file="", title="", align='rcc', 
# 		# n.cgroup=c(1, dim(chrt_SIMEX_smry_B)[2]-1), cgroup=c("", "Grades"),
# 		caption='SIMEX Corrected EOCT Median and Mean <em>Cohort</em> Referenced Student Growth Percentile by Content Area for 2014'))
# 
#   cat("\n<!-- LaTeX_Start\n")
#   print(latex(as.matrix(chrt_SIMEX_smry_B), file="", title="", align='rcc', 
#   	caption='SIMEX Corrected EOCT Median and Mean <em>Cohort</em> Referenced Student Growth Percentile by Content Area for 2014'))
#   cat("\nLaTeX_End -->\n")
    
#  z <- ztable(as.matrix(chrt_SIMEX_smry_B), align='rcc', type="latex",
#         caption='SIMEX Corrected EOCT Median and Mean <em>Cohort</em> Referenced Student Growth Percentile by Content Area for 2014')
```
...
```{r, results='asis', echo=FALSE, baseSumCRCTsimex}
  ...
  cat(dualTable(as.matrix(bsln_crct_SIMEXB), align=paste(rep('r', dim(bsln_crct_SIMEXB)[2]), collapse=''), 
...
#   cat("\n<!-- HTML_Start -->\n")
# # 	options("table_counter"=getOption("table_counter")-1)
#   cat(htmlTable(as.matrix(bsln_crct_SIMEXB), file="", title="", align=paste(rep('r', dim(bsln_crct_SIMEXB)[2]), collapse=''), 
# 		n.cgroup=c(1, dim(bsln_crct_SIMEXB)[2]-1), cgroup=c("", "Grades"),
# 		caption='SIMEX Corrected CRCT Median (Mean) <em>Baseline</em> Student Growth Percentile by Grade and Content Area for 2014'))
# 
#   cat("\n<!-- LaTeX_Start\n")
#   print(latex(as.matrix(bsln_crct_SIMEXB), file="", title="", align=paste(rep('r', dim(bsln_crct_SIMEXB)[2]), collapse=''), 
#     n.cgroup=c(1, dim(bsln_crct_SIMEXB)[2]-1), cgroup=c("", "Grades"),
# 		caption='SIMEX Corrected CRCT Median (Mean) <em>Baseline</em> Student Growth Percentile by Grade and Content Area for 2014'))
#   cat("\nLaTeX_End -->\n")
#     
#  z <- ztable(as.matrix(bsln_crct_SIMEXB), align='rcc', type="latex",
#         caption='SIMEX Corrected CRCT Median (Mean) <em>Baseline</em> Student Growth Percentile by Grade and Content Area for 2014')
#  z <- addcgroup(z, cgroup=c("", "Grades"), n.cgroup=c(1, dim(bsln_crct_SIMEXB)[2]-1))    				
#  closeAllConnections()
```
...
```{r, results='asis', echo=FALSE, bslnSmrySIMEX}
	...
	bsln_eoct_SIMEX_B <- bsln_eoct_SIMEX_B[, CONTENT_AREA := sapply(CONTENT_AREA, capwords, special.words=c("II", "US"))]
	setnames(bsln_eoct_SIMEX_B, "CONTENT_AREA", "Content Area")

  cat(dualTable(as.matrix(bsln_eoct_SIMEX_B), align='rcc', 
		caption='SIMEX Corrected EOCT Median and Mean <em>Baseline</em> Referenced Student Growth Percentile by Content Area for 2014'))

#   cat("\n<!-- HTML_Start -->\n")
# #   options("table_counter"=getOption("table_counter")-1)
# 	cat(htmlTable(as.matrix(bsln_eoct_SIMEX_B), file="", title="", align='rcc', 
# 		# n.cgroup=c(1, dim(bsln_eoct_SIMEX_B)[2]-1), #cgroup=c("", "Grades"),
# 		caption='SIMEX Corrected EOCT Median and Mean <em>Baseline</em> Referenced Student Growth Percentile by Content Area for 2014'))
# 
#   cat("\n<!-- LaTeX_Start\n")
#   print(latex(as.matrix(bsln_eoct_SIMEX_B), file="", title="", align='rcc', 
# 		caption='SIMEX Corrected EOCT Median and Mean <em>Baseline</em> Referenced Student Growth Percentile by Content Area for 2014'))
#   cat("\nLaTeX_End -->\n")
#     
#  z <- ztable(as.matrix(bsln_eoct_SIMEX_B), align='rcc', type="latex",
#    caption='SIMEX Corrected EOCT Median and Mean <em>Baseline</em> Referenced Student Growth Percentile by Content Area for 2014')
```
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, GOFplots}
	##
  ##    Create goodness of fit plots for tech report examples and Appendix A
  ##

	dir.create("img/Examples", recursive=TRUE, showWarnings=FALSE)
...
SIMEX corrected SGPs induce a negative correlation between growth and prior achievement (regardless of reference type) as shown in Table `r tblNumNext()` below.  Rather than a uniform distribution, SIMEX produces a distribution in which growth for lower prior achieving students' growth is weighted upward and higher prior achieving students' growth is weighted down.  In theory this produces biased student-level SGPs but may decrease the bias in aggregate growth measures as has been documented previous SIMEX reports.  This in turn decreases the observed correlations between "uncorrected" SGPs and prior achievement^[Note that aggregate-level correlations that are initially negative also become increasingly negative rather than return to zero.  This has been noted in test results from other states not presented here.].
...
```{r, results='asis', echo=FALSE, Student_CRCT}
	tmp_tbl <- data.frame(student.cor.grd[CONTENT_AREA %in% crct.subj][order(match(student.cor.grd$CONTENT_AREA, crct.subj))][!is.na(BASELINE_SGP)]) # [, COHORT_SIMEX := NULL]
	...
	n_col_group <- c(1, dim(tmp_tbl)[2]-3, 1) # -3 :: 2 for CONTENT_AREA and GRADE, and 1 for N.
	col_group <- c("", "Aggregate Group Type", "")
	###  Number of "row groups" - here I'm breaking on Content area.  
	n_row_group <- diff(c(match(crct.subj, tmp_tbl$CONTENT_AREA), dim(tmp_tbl)[1]+1))
	row_group <- sapply(unique(tmp_tbl$CONTENT_AREA), capwords, USE.NAMES=FALSE)
	tmp_capt <- "CRCT Correlations between Student-Level Prior Standardized Scale Score and SGP Versions"
	tmp_tbl <- tmp_tbl[, -1]
	names(tmp_tbl) <- sapply(names(tmp_tbl), capwords)

  cat("\n<!-- HTML_Start -->\n")
#   options("table_counter"=getOption("table_counter")-1)
	suppressWarnings(cat(htmlTable(as.matrix(prettyNum(tmp_tbl, preserve.width = "individual", big.mark=',')), 
		title="", align=paste(rep('r', dim(tmp_tbl)[2]), collapse=''),
		n.cgroup=n_col_group, cgroup= col_group, n.rgroup=n_row_group, rgroup=row_group, compatibility="CSS", caption = tmp_capt)))

#  print(latex(as.matrix(prettyNum(tmp_tbl, preserve.width = "individual", big.mark=',')), 
#    file="", title="", align=paste(rep('r', dim(tmp_tbl)[2]), collapse=''),
#		n.cgroup=n_col_group, cgroup= col_group, n.rgroup=n_row_group, rgroup=row_group, caption = 'SIMEX Corrected EOCT Median and Mean <em>Baseline</em> Referenced Student Growth Percentile by Content Area for 2014'))
    
  cat("\n<!-- LaTeX_Start\n")
  z <- ztable(as.matrix(prettyNum(tmp_tbl, preserve.width = "individual", big.mark=',')), type="latex",
    align=paste(rep('r', dim(tmp_tbl)[2]), collapse=''),include.rownames=FALSE, 
    caption=tblCap(tmp_capt))
#     caption=capt <- paste("Table ", getOption("table_counter"),": ", tmp_capt, sep=""))
# z <- addcgroup(z, cgroup=col_group, n.cgroup=n_col_group)    				
  z <- addrgroup(z, rgroup=row_group, n.rgroup=n_row_group, cspan.rgroup=1)
  print(z)
#   options("table_counter"=getOption("table_counter")+1)
  cat("\nLaTeX_End -->\n")
...
```{r, results='asis', echo=FALSE, Student_EOCTBaseline}
	...
	n_row_group <- NULL
	row_group <- NULL
	tmp_capt <- "Correlations between Student-Level Prior Standardized Scale Score and EOCT Baseline Referenced SGP Versions"
	tmp_tbl <- tmp_tbl[, -2]
	tmp_tbl$CONTENT_AREA <- sapply(tmp_tbl$CONTENT_AREA, capwords, special.words=c("II", "US"))
	names(tmp_tbl) <- sapply(names(tmp_tbl), capwords)

  cat(dualTable(as.matrix(prettyNum(tmp_tbl, preserve.width = "individual", big.mark=',')), 
		align=paste(rep('r', dim(tmp_tbl)[2]), collapse=''), 
		n.cgroup=n_col_group, cgroup= col_group, compatibility="CSS", caption = tmp_capt))

#   cat("\n<!-- HTML_Start -->\n")
# #   options("table_counter"=getOption("table_counter")-1)
# 	cat(htmlTable(as.matrix(prettyNum(tmp_tbl, preserve.width = "individual", big.mark=',')), 
# 		file="", title="", align=paste(rep('r', dim(tmp_tbl)[2]), collapse=''), 
# 		n.cgroup=n_col_group, cgroup= col_group, compatibility="CSS", caption = tmp_capt))
# 
#   cat("\n<!-- LaTeX_Start\n")
...
#  z <- ztable(as.matrix(prettyNum(tmp_tbl, preserve.width = "individual", big.mark=',')), 
#    align=paste(rep('r', dim(tmp_tbl)[2]), collapse=''),include.rownames=FALSE, caption=tmp_capt)
#  z <- addcgroup(z, cgroup=col_group, n.cgroup=n_col_group)    				
```
<p></p>
...
```{r, results='asis', echo=FALSE, Student_EOCTCohort}
	...
	###  Number of "column groups" - here different aggregates correlated with standardized prior score (and blank for content area and N)
	n_col_group <- c(1, dim(tmp_tbl)[2]-2, 1) # -2 :: 1 for CONTENT_AREA, and 1 for N.
	col_group <- c("", "Aggregate Group Type", "")
	###  Number of "row groups" - none here.  
	n_row_group <- NULL
	row_group <- NULL
	tmp_capt <- "Correlations between Student-Level Prior Standardized Scale Score and EOCT Cohort Referenced SGP Versions"
	tmp_tbl$CONTENT_AREA <- sapply(tmp_tbl$CONTENT_AREA, capwords, special.words=c("II", "US"))
	names(tmp_tbl) <- sapply(names(tmp_tbl), capwords)

  cat(dualTable(as.matrix(prettyNum(tmp_tbl, preserve.width = "individual", big.mark=',')), 
		where="!htbp", align=paste(rep('r', dim(tmp_tbl)[2]), collapse=''), 
		n.cgroup=n_col_group, cgroup= col_group, compatibility="CSS", caption = tmp_capt))

#   cat("\n<!-- HTML_Start -->\n")
# #   options("table_counter"=getOption("table_counter")-1)
# 	cat(htmlTable(as.matrix(prettyNum(tmp_tbl, preserve.width = "individual", big.mark=',')), 
# 		file="", title="", where="!htbp", align=paste(rep('r', dim(tmp_tbl)[2]), collapse=''), 
# 		n.cgroup=n_col_group, cgroup= col_group, compatibility="CSS", caption = tmp_capt))
# 
#   cat("\n<!-- LaTeX_Start\n")
#   print(latex(as.matrix(prettyNum(tmp_tbl, preserve.width = "individual", big.mark=',')), 
...
#  cat("\n<!-- HTML_Start -->\n")
#   options("table_counter"=getOption("table_counter")-1)
#	cat(htmlTable(as.matrix(tmp_tbl), file="", title="", align=paste(rep('r', dim(tmp_tbl)[2]), collapse=''), 
#		n.cgroup=n_col_group, cgroup= col_group, compatibility="CSS", caption  = tmp_capt))
...
```{r, results='asis', echo=FALSE, School_CRCT_Correlations}
	...
	n_col_group <- c(1, dim(tmp_tbl)[2]-2) # -1 for CONTENT_AREA, -1 for YEAR
	col_group <- c("", "Aggregate SGP Type")	
	###  Number of "row groups" - none here.  
	n_row_group <- diff(c(match(crct.subj, tmp_tbl$CONTENT_AREA), dim(tmp_tbl)[1]+1))
	row_group <- sapply(unique(tmp_tbl$CONTENT_AREA), capwords, special.words=c("II", "US"))
	tmp_capt <- "CRCT Correlations between School-Level Mean Prior Standardized Scale Score and (Baseline) SGP Aggregations."	
...
#  cat("\n<!-- HTML_Start -->\n")
#  options("table_counter"=getOption("table_counter")-1)
#	suppressWarnings(cat(htmlTable(as.matrix(tmp_tbl), file="", title="", align=paste(rep('r', dim(tmp_tbl)[2]), collapse=''), 
#		n.cgroup=n_col_group, cgroup= col_group, n.rgroup=n_row_group, rgroup= row_group, compatibility="CSS", caption  = tmp_capt)))
...
```{r, results='asis', echo=FALSE, School_Grade_CRCT_Correlations}
	...
	###  Number of "column groups" - here different aggregates correlated with standardized prior score
	n_col_group <- c(1, dim(tmp_tbl)[2]-2) # -2 for both CONTENT_AREA and GRADE.  C_A nulled out below.
	col_group <- c("", "Aggregate Group Type")
	###  Number of "row groups" - here I'm breaking on Content area.  
	n_row_group <- diff(c(match(crct.subj, tmp_tbl$CONTENT_AREA), dim(tmp_tbl)[1]+1))
	row_group <- sapply(unique(tmp_tbl$CONTENT_AREA), capwords, special.words=c("II", "US"))
	tmp_capt <- "2014 CRCT Correlations between School-Level prior standardized scale score and Aggregate (Baseline) SGPs by Grade."
	tmp_tbl <- tmp_tbl[, -1]
	names(tmp_tbl) <- sapply(names(tmp_tbl), capwords)
...
```{r, results='asis', echo=FALSE, School_EOCTBaseline_Cor}
	...
	col_group <- c("", "Aggregate SGP Type")	
	###  Number of "row groups" - none here.  
	n_row_group <- diff(c(match(eoct.subj[eoct.sgp.ref == "BASE"], tmp_tbl$CONTENT_AREA), dim(tmp_tbl)[1]+1))
	row_group <- sapply(unique(tmp_tbl$CONTENT_AREA), capwords, special.words=c("II", "US"))
	tmp_capt <- "2011 to 2014 Correlations between Mean Prior Standardized Scale Score and Aggregate Baseline SGPs."
	tmp_tbl$CONTENT_AREA <- NULL
	names(tmp_tbl) <- sapply(names(tmp_tbl), capwords)
	tmp_tbl[is.na(tmp_tbl)] <- ""
...
```{r, results='asis', echo=FALSE, School_EOCTCohort_Cor}
  ...
  tmp_tbl <- data.frame(sch.cor.subj.cohort[order(match(sch.cor.subj.cohort$CONTENT_AREA, eoct.subj)),][CONTENT_AREA %in% eoct.subj[eoct.sgp.ref == "COHORT"]])
	###  Number of "column groups" - here different aggregates correlated with standardized prior score
	n_col_group <- c(1, dim(tmp_tbl)[2]-2) # -1 for CONTENT_AREA, -1 for YEAR
	col_group <- c("", "Aggregate SGP Type")	
	###  Number of "row groups" - none here.  
	n_row_group <- diff(c(match(eoct.subj[eoct.sgp.ref == "COHORT"], tmp_tbl$CONTENT_AREA), dim(tmp_tbl)[1]+1))
	row_group <- sapply(unique(tmp_tbl$CONTENT_AREA), capwords, special.words=c("II", "US"))
	tmp_capt <- "2011 to 2014 Correlations between Mean Prior Standardized Scale Score and Cohort SGPs."
	tmp_tbl$CONTENT_AREA <- NULL
	names(tmp_tbl) <- sapply(names(tmp_tbl), capwords)
	tmp_tbl[is.na(tmp_tbl)] <- ""
