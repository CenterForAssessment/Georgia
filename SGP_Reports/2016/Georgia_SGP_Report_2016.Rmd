...
date: October 2016
abstract: "DRAFT REPORT - DO NOT CITE! <br></br> This report provides details about the Georgia Student Growth Model methodology and presents a descriptive analysis of the 2016 SGP calculation process and results."
---
...
This document was written by Damian Betebenner & Adam VanIwaarden for the State of Georgia Department of Education (DOE).
	...
	Original Draft:  October 6, 2016
	Final Draft:     October 20, 2016
-->


```{r, echo=FALSE, include=FALSE}
  ## set a universal Cache path
  knitr::opts_chunk$set(cache.path = "_cache/GA_SGP_2016")
...
Data for the Georgia Milestones end-of-grade (EOG) and end-of-course (EOC) tests used in the SGP analyses were supplied by the Georgia DOE to the NCIEA for analysis in the fall of 2016.  These Milestones test records were added to existing Georgia Criterion-Referenced Competency Tests (CRCT) and EOC test assessment program data from the 2011-2012 through 2013-2014 academic years and the 2014-2015 Milestones data to create the longitudinal data set from which the 2016 SGPs were calculated.  Subsequent years' analyses will augment this multi-year data set allowing DOE to maintain a comprehensive longitudinal data set for all students taking the EOG and EOC Milestones assessments.
...
This section provides basic details about the calculation of student growth percentiles from Georgia state assessment data using the [`R` Software Environment](http://www.r-project.org/) [@Rsoftware] in conjunction with the [`SGP` package](https://github.com/CenterForAssessment/SGP) [@sgp2016].  More in depth treatment of the data analysis process with code examples has been provided to Georgia DOE staff in the 2016 Calculation Manual.
...
The 2016 Georgia SGP analyses follow a work flow established in previous years that includes the following 4 steps:
...
As an example, Figure `r getOption("fig_caption_no")+1` shows the results for 8<sup>th</sup> grade ELA as an example of good model fit.  Figure `r getOption("fig_caption_no")+2` is the fit plot for one of the Analytic Geometry analyses that incorporates a skipped year (i.e. students with no 2015 mathematics scores) and demonstrates minor model misfit.
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, GOFplots}
  ...
  dat <- Georgia_SGP@Data[SGP_NORM_GROUP %in% "2014/READING_6; 2014/ELA_6; 2015/ELA_7; 2016/ELA_8", 
  				 c("VALID_CASE", "CONTENT_AREA", "GRADE", "YEAR", "ID", "SGP", "SGP_SIMEX", "SCALE_SCORE", "SCALE_SCORE_PRIOR", "SGP_NORM_GROUP"), with=FALSE]

	gofSGP(dat, state="GA", years='2016', content_areas="ELA", use.sgp="SGP", output.format="PNG")
	gofSGP(dat, state="GA", years='2016', content_areas="ELA", use.sgp="SGP_SIMEX", output.format="PNG")

  dat <- Georgia_SGP@Data[SGP_NORM_GROUP %in% "2014/COORDINATE_ALGEBRA_EOCT; 2016/ANALYTIC_GEOMETRY_EOCT", 
  				 c("VALID_CASE", "CONTENT_AREA", "GRADE", "YEAR", "ID", "SGP", "SGP_SIMEX", "SCALE_SCORE", "SCALE_SCORE_PRIOR", "SGP_NORM_GROUP"), with=FALSE]

	gofSGP(dat, state="GA", years='2016', content_areas='ANALYTIC_GEOMETRY', use.sgp="SGP", output.format="PNG")
	gofSGP(dat, state="GA", years='2016', content_areas='ANALYTIC_GEOMETRY', use.sgp="SGP_SIMEX", output.format="PNG")
...
##### `r figCapNo("Goodness of Fit Plot for 2016 ***Uncorrected*** 8<sup>th</sup> Grade ELA:  Example of good model fit.")`
![](../img/Goodness_of_Fit/ELA.2016/gofSGP_Grade_8.png)
...
Minor misfit in the Analytic Geometry model is likely due to the relatively small cohort size, as well as the skipped year in the course progression (2014 Coordinate Algebra is the only prior used in this analysis).  The indication of a potential ceiling effect in this plot is probably overstated given the range of scores included in the HOSS (bottom) row.  That is, the low SGPs are likely attributed to students scoring just below the HOSS, and therefore less concerning.  This situation is investigated in greater detail in Appendix C of this report.

##### `r figCapNo("Goodness of Fit Plot for a 2016 ***Uncorrected*** Analytic Geometry Progression: Example of slight model mis-fit.")`
![](../img/Goodness_of_Fit/ANALYTIC_GEOMETRY.2016/2016_ANALYTIC_GEOMETRY_EOCT;2014_COORDINATE_ALGEBRA_EOCT.png)
...
```{r, results='asis', echo=FALSE, Student_EOG}
	student.cor.grd <- Georgia_SGP@Data[YEAR=='2016' & VALID_CASE=='VALID_CASE' & CONTENT_AREA %in% GL_subjects][, list(
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, Student_EOCT}
	student.cor.eoct <- Georgia_SGP@Data[YEAR=='2016' & VALID_CASE=='VALID_CASE' & CONTENT_AREA %in% EOC_subjects][, list(
		`$\\\rr_ {  Test Scores}$` = round(cor(SCALE_SCORE, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 2), 
		`$\\\rr_ {  SGP}$` = round(cor(SGP, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 2), 
...
```{r, results='asis', echo=FALSE, Student_EOCT_2}
	eoct_tmp_tbl <- student.cor.eoct[!is.na(student.cor.eoct[["$\\\rr_ {  SGP}$"]]) & CONTENT_AREA %in% c("BIOLOGY","PHYSICAL_SCIENCE","COORDINATE_ALGEBRA","ANALYTIC_GEOMETRY", "ALGEBRA_I", "GEOMETRY")] 
...
```{r, results='asis', echo=FALSE, SumEOG}
	EOG_smry <- Georgia_SGP@Data[CONTENT_AREA %in% GL_subjects & YEAR=='2016'][, list(MEDIAN = median(as.numeric(SGP), na.rm=TRUE), MEAN = round(mean(SGP, na.rm=TRUE), 1)), by=c('CONTENT_AREA', 'GRADE')][!is.na(MEDIAN)]
	setkey(EOG_smry)
  ...
  cat(dualTable(as.matrix(EOG_smryB), title="", n.cgroup=c(1, dim(EOG_smryB)[2]-1), cgroup=c("", "Grades"),
  	align=paste(rep('r', dim(EOG_smryB)[2]), collapse=''),
		caption='2016 EOG Median (Mean) Student Growth Percentile by Grade and Content Area.'))
```

```{r, results='asis', echo=FALSE, SumEOC}
	EOC_smry <- Georgia_SGP@Data[CONTENT_AREA %in% EOC_subjects & YEAR=='2016'][,list("Median SGP"=median(as.numeric(SGP), na.rm=TRUE), "Mean SGP"=round(mean(SGP, na.rm=TRUE), 1)), by='CONTENT_AREA']
	setkey(EOC_smry)
  ...
  cat(dualTable(as.matrix(EOC_smry_B), align='rcc',
		caption='2016 EOC Median and Mean Student Growth Percentile by Content Area.'))
```
...
```{r, results='asis', echo=FALSE, SIMEX_SumEOG}
	EOG_SIMEX_smry <- Georgia_SGP@Data[CONTENT_AREA %in% GL_subjects & YEAR=='2016'][,list("Median SGP"=median(as.numeric(SGP_SIMEX), na.rm=TRUE), "Mean SGP"=round(mean(SGP_SIMEX, na.rm=TRUE), 1)), by=c('CONTENT_AREA', 'GRADE')]
  ...
  cat(dualTable(as.matrix(EOG_SIMEX_smryB), title="", n.cgroup=c(1, dim(EOG_SIMEX_smryB)[2]-1), cgroup=c("", "Grades"),
  	align=paste(rep('r', dim(EOG_SIMEX_smryB)[2]), collapse=''),
		caption='SIMEX Corrected EOG Median (Mean) Student Growth Percentile by Grade and Content Area for 2016'))
```


```{r, results='asis', echo=FALSE, SIMEX_SumEOC}
	EOC_SIMEX_smry <- Georgia_SGP@Data[CONTENT_AREA %in% EOC_subjects & YEAR=='2016'][,list("Median SGP"=median(as.numeric(SGP_SIMEX), na.rm=TRUE), "Mean SGP"=round(mean(SGP_SIMEX, na.rm=TRUE), 1)), by='CONTENT_AREA']
  ...
  cat(dualTable(as.matrix(EOC_SIMEX_smry_B), align='rcc',
		caption='SIMEX Corrected EOC Median (Mean) Student Growth Percentile by Content Area for 2016'))
```
...
##### `r figCapNo("School-level Bubble Plots for Georgia: ELA, 2015-2016.")`
![](../img/Bubble_Plots/2016/State/Style_1/Georgia_2016_ELA_State_Bubble_Plot_(Prior_Achievement).png)

<p></p>
<p></p>


##### `r figCapNo("School-level Bubble Plots for Georgia: Mathematics, 2015-2016.")`
![](../img/Bubble_Plots/2016/State/Style_1/Georgia_2016_Mathematics_State_Bubble_Plot_(Prior_Achievement).png)

<p></p>
<p></p>

##### `r figCapNo("School-level Bubble Plots for Georgia: Science, 2015-2016.")`
![](../img/Bubble_Plots/2016/State/Style_1/Georgia_2016_Science_State_Bubble_Plot_(Prior_Achievement).png)
...
##### `r figCapNo("School-level Bubble Plots for Georgia: Social Studies, 2015-2016.")`
![](../img/Bubble_Plots/2016/State/Style_1/Georgia_2016_Social Studies_State_Bubble_Plot_(Prior_Achievement).png)
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	sch.msgp <- Georgia_SGP@Summary$SCHOOL_NUMBER$SCHOOL_NUMBER__YEAR__SCHOOL_ENROLLMENT_STATUS[YEAR %in% c('2014', '2015', '2016') & !is.na(SCHOOL_NUMBER) & MEDIAN_SGP_COUNT > 14] # Last 3 YEARs
	sch.msgp.subj <- Georgia_SGP@Summary$SCHOOL_NUMBER$SCHOOL_NUMBER__CONTENT_AREA__YEAR__SCHOOL_ENROLLMENT_STATUS[YEAR %in% c('2014', '2015', '2016') & !is.na(SCHOOL_NUMBER) & MEDIAN_SGP_COUNT > 14] # Last 3 YEARs
	sch.msgp.grd <- Georgia_SGP@Summary$SCHOOL_NUMBER$SCHOOL_NUMBER__CONTENT_AREA__YEAR__GRADE__SCHOOL_ENROLLMENT_STATUS[YEAR == '2016' & !is.na(SCHOOL_NUMBER) & MEDIAN_SGP_COUNT > 14]
	...
	sch.cor <- sch.msgp[, list(
			...
			MEAN_SIMEX_SGP = round(cor(MEAN_SGP_SIMEX, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2)), keyby = 'YEAR']
	setkey(sch.cor, YEAR)
	...
	sch.cor.subj <- sch.msgp.subj[, list(
			...
			MEAN_SGP = round(cor(MEAN_SGP, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
			MEDIAN_SIMEX_SGP = round(cor(MEDIAN_SGP_SIMEX, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
			MEAN_SIMEX_SGP = round(cor(MEAN_SGP_SIMEX, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2)), keyby = c('CONTENT_AREA', 'YEAR')]

	## 2016 by grade - School_Grade_EOG_Correlations
	sch.cor.grd <- sch.msgp.grd[, list(
		MEDIAN_SGP = round(cor(MEDIAN_SGP, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
		MEAN_SGP = round(cor(MEAN_SGP, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
		MEDIAN_SIMEX_SGP = round(cor(MEDIAN_SGP_SIMEX, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2),
		MEAN_SIMEX_SGP = round(cor(MEAN_SGP_SIMEX, MEAN_SCALE_SCORE_PRIOR_STANDARDIZED), 2)), keyby = c('CONTENT_AREA', 'GRADE')]
...
```{r, results='asis', echo=FALSE, School_Cor_Grand}
	...
	n_row_group <- NULL
	row_group <- NULL
	tmp_capt <- "2014 to 2016 Correlations between Mean Prior Standardized Scale Score and Aggregate SGPs - (Combined Subjects)"
