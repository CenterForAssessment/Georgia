...
subtitle: "A Technical Overview of the 2016-2017 Student Growth Percentile Calculations"
author:
...
date: October 2017
abstract: "DRAFT REPORT - DO NOT CITE! <br></br> This report provides details about the Georgia Student Growth Model methodology and presents a descriptive analysis of the 2017 SGP calculation process and results."
---
...
This document was written by Damian Betebenner & Adam VanIwaarden for the State of Georgia Department of Education (DOE).
	...
	Original Draft:  October 3, 2017
	Final Draft:     October 5, 2017
-->


```{r, echo=FALSE, include=FALSE}
  ## set a universal Cache path
  knitr::opts_chunk$set(cache.path = "_cache/GA_SGP_2017")
...
- ***Analytics - *** introduces some of the basic statistical methods and the computational process implemented in the 2017 analyses.^[More in-depth treatment of the SGP Methodology can be found [here](https://github.com/CenterForAssessment/SGP_Resources/tree/master/articles).]
...
Data for the Georgia Milestones end-of-grade (EOG) and end-of-course (EOC) tests used in the SGP analyses were supplied by the Georgia DOE to the NCIEA for analysis in late summer of 2017.  These Milestones test records were added to existing Georgia assessment data^[2017 Prior assessment data includes Criterion-Referenced Competency Tests (CRCT) and EOCT assessment program data from the 2013-2014 academic year and the 2014-2015 through 2015-2016 Milestones data] to create the longitudinal data set from which the 2017 SGPs were calculated.  Subsequent years' analyses will augment this multi-year data set allowing DOE to maintain a comprehensive longitudinal data set for all students taking the EOG and EOC Milestones assessments.
...
Table `r tblNum(1)` shows the number of valid EOG student records available for analysis after applying the general and EOG specific business rules.^[This number does not represent the number of SGPs produced, however, because students are required to have at least one prior score available as well.]

```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_EOG <- table(Georgia_SGP@Data[YEAR=='2017' & CONTENT_AREA %in% GL_subjects & VALID_CASE=='VALID_CASE']$CONTENT_AREA, Georgia_SGP@Data[YEAR=='2017' & CONTENT_AREA %in% GL_subjects & VALID_CASE=='VALID_CASE']$GRADE)
```
```{r, results='asis', echo=FALSE, N_tableEOG}
  ...
  cat(dualTable(as.matrix(n_tbl_EOG_B), 
  	align=paste(rep('r', dim(n_tbl_EOG_B)[2]), collapse=''),
		...
		caption='Number of Valid EOG Student Records by Grade and Subject for 2017'))
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	n_tbl_EOC <- table(Georgia_SGP@Data[YEAR=='2017' & CONTENT_AREA %in% EOC_subjects & VALID_CASE=='VALID_CASE']$CONTENT_AREA, Georgia_SGP@Data[YEAR=='2017' & CONTENT_AREA %in% EOC_subjects & VALID_CASE=='VALID_CASE']$GRADE)
```
```{r, results='asis', echo=FALSE, N_tableEOC}
  ...
  cat(dualTable(as.matrix(n_tbl_EOC_B), align=paste(rep('r', dim(n_tbl_EOC_B)[2]), collapse=''), 
		caption='Total Number of Valid EOC Student Records by Subject for 2017'))
...
This section provides basic details about the calculation of student growth percentiles from Georgia state assessment data using the [`R` Software Environment](http://www.r-project.org/) [@Rsoftware] in conjunction with the [`SGP` package](https://github.com/CenterForAssessment/SGP) [@sgp2017].  More in depth treatment of the data analysis process with code examples has been provided to Georgia DOE staff in the 2017 Calculation Manual.
...
In this step the data from step 1a is read into [`R`](http://www.r-project.org/) and modified slightly. The result is a `.Rdata` file containing data in the format suitable for analysis with the [`SGP` package](https://github.com/CenterForAssessment/SGP).  This data is combined with a select subset of CRCT, EOCT and Milestones data necessary to complete the 2017 and future Milestones analyses stored in a new [`SGP` class object](http://cran.r-project.org/web/packages/SGP/SGP.pdf).  With an appropriate longitudinal data set prepared, we move to the calculation of student-level SGPs.
...
Due to differences in the time-frames in which the EOG and EOC were validated and made available, EOG and EOC SGPs were calculated at seperate times.  Cohort-referenced (uncorrected) and SIMEX corrected SGPs were calculated concurrently for each individual analysis.^[In addition to these quantities, baseline-referenced SGPs (uncorrected and SIMEX corrected) had been calculated in previous years and used as the "official" SGP quantity, but this method of SGP calculation will be unavailable until at least 3 years of Milestones assessment data are available for all analyses to form the baseline cohorts.]  We use the [`updateSGP`](https://www.rdocumentation.org/packages/SGP/versions/1.7-0.0/topics/updateSGP) function to ***a)*** do the final preparation and addition of the 2017 cleaned and formatted data to the new `SGP` class object ([`prepareSGP`](https://www.rdocumentation.org/packages/SGP/versions/1.7-0.0/topics/prepareSGP) step) and ***b)*** calculate SGP estimates ([`analyzeSGP`](https://www.rdocumentation.org/packages/SGP/versions/1.7-0.0/topics/analyzeSGP) step).  
...
Once all analyses were completed the results were merged into the master longitudinal data set ([`combineSGP`](https://www.rdocumentation.org/packages/SGP/versions/1.7-0.0/topics/combineSGP) step).  A pipe delimited version of the complete long data is output ([`outputSGP`](https://www.rdocumentation.org/packages/SGP/versions/1.7-0.0/topics/outputSGP) step) and submitted to the Georgia DOE after some additional formatting to add fields needed for rendering data in the state's [visualization tool](http://gastudentgrowth.gadoe.org/) such as students' entire prior score and course progression history.  The data is also summarized using the [`summarizeSGP` function](https://www.rdocumentation.org/packages/SGP/versions/1.7-0.0/topics/summarizeSGP), which produces many tables of descriptive statistics that are disaggregated at the state, district, school and instructor levels, as well as other factors of interest.  Finally, visualizations (such as bubble charts) are produced from the data and summary tables using the [`visualizeSGP`](https://www.rdocumentation.org/packages/SGP/versions/1.7-0.0/topics/visualizeSGP) function. 
...
As an example, Figure `r getCounter("figure")+1` shows the results for 8<sup>th</sup> grade ELA as an example of good model fit.  Figure `r getCounter("figure")+2` is an example of minor model misfit from the Geometry course/test repeater analysis (i.e. students have Geometry scores in 2016 and 2017).
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, GOFplots}
  ...
  dat <- Georgia_SGP@Data[SGP_NORM_GROUP %in% "2015/ELA_6; 2016/ELA_7; 2017/ELA_8", 
	...
	gofSGP(dat, state="GA", years='2017', content_areas="ELA", use.sgp="SGP", output.format="PNG")
	# gofSGP(dat, state="GA", years='2017', content_areas="ELA", use.sgp="SGP_SIMEX", output.format="PNG")
	gofSGP(dat, state="GA", years='2017', content_areas="ELA", use.sgp="SGP_SIMEX_RANKED", output.format="PNG")) # Run before SIMEX and Rename folder???

  dat <- Georgia_SGP@Data[SGP_NORM_GROUP %in% "2016/GEOMETRY_EOCT; 2017/GEOMETRY_EOCT",
  				 c("VALID_CASE", "CONTENT_AREA", "GRADE", "YEAR", "ID", "SGP", "SGP_SIMEX", "SGP_SIMEX_RANKED", "SCALE_SCORE", "SCALE_SCORE_PRIOR", "SGP_NORM_GROUP"), with=FALSE]

	gofSGP(dat, state="GA", years='2017', content_areas='GEOMETRY', use.sgp="SGP", output.format="PNG")
	# gofSGP(dat, state="GA", years='2017', content_areas='GEOMETRY', use.sgp="SGP_SIMEX", output.format="PNG")
	gofSGP(dat, state="GA", years='2017', content_areas='GEOMETRY', use.sgp="SGP_SIMEX_RANKED", output.format="PNG") # Rename folder???
...
```{r, results="asis", echo=FALSE, G8_ELA_GoF_Cohort}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/ELA.2017/gofSGP_Grade_8.png",
...
```{r, results="asis", echo=FALSE, Geom_GoF_Cohort}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/GEOMETRY.2017/2017_GEOMETRY_EOCT;2016_GEOMETRY_EOCT.png",
			caption = "Goodness of Fit Plot for a 2017 ***Uncorrected*** Geometry Progression: Example of slight model mis-fit.")
```
...
```{r, results="asis", echo=FALSE, G8_ELA_GoF_Ranked_SIMEX}
		placeFigure(page.break= TRUE,
			files = "../img/Goodness_of_Fit/ELA.2017.SIMEX/gofSGP_Grade_8.png",
			caption = "Goodness of Fit Plot for 2017 ***Ranked SIMEX Corrected*** 8<sup>th</sup> Grade ELA:  Example of good model fit.")
```

```{r, results="asis", echo=FALSE, Geom_GoF_Ranked_SIMEX}
		placeFigure(
			files = "../img/Goodness_of_Fit/GEOMETRY.2017.SIMEX/2017_GEOMETRY_EOCT;2016_GEOMETRY_EOCT.png",
			caption = "Goodness of Fit Plot for a 2017 ***Ranked SIMEX Corrected*** Geometry Progression.")
```
...
```{r, results='asis', echo=FALSE, Student_EOG}
	student.cor.grd <- Georgia_SGP@Data[YEAR=='2017' & VALID_CASE=='VALID_CASE' & CONTENT_AREA %in% GL_subjects][, list(
...
```{r, cache=TRUE, echo=FALSE, include=FALSE, Student_EOCT}
	student.cor.eoct <- Georgia_SGP@Data[YEAR=='2017' & VALID_CASE=='VALID_CASE' & CONTENT_AREA %in% EOC_subjects][, list(
		`$\\\rr_ {  Test Scores}$` = round(cor(SCALE_SCORE, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 2), 
		`$\\\rr_ {  SGP}$` = round(cor(SGP, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 2), 
		`$\\\rr_ { Ranked SIMEX}$` = round(cor(SGP_SIMEX_RANKED, SCALE_SCORE_PRIOR_STANDARDIZED, use='pairwise.complete'), 2), 
...
```{r, results='asis', echo=FALSE, SumEOG}
	EOG_smry <- Georgia_SGP@Data[CONTENT_AREA %in% GL_subjects & YEAR=='2017'][, list(MEDIAN = median(as.numeric(SGP), na.rm=TRUE), MEAN = round(mean(SGP, na.rm=TRUE), 1)), by=c('CONTENT_AREA', 'GRADE')][!is.na(MEDIAN)]
	setkey(EOG_smry)
  ...
  cat(dualTable(as.matrix(EOG_smryB), title="", n.cgroup=c(1, dim(EOG_smryB)[2]-1), cgroup=c("", "Grades"),
  	align=paste(rep('r', dim(EOG_smryB)[2]), collapse=''),
		caption='2017 EOG Median (Mean) Student Growth Percentile by Grade and Content Area.'))
```

```{r, results='asis', echo=FALSE, SumEOC}
	EOC_smry <- Georgia_SGP@Data[CONTENT_AREA %in% EOC_subjects & YEAR=='2017'][,list("Median SGP"=median(as.numeric(SGP), na.rm=TRUE), "Mean SGP"=round(mean(SGP, na.rm=TRUE), 1)), by='CONTENT_AREA']
	setkey(EOC_smry)
  ...
  cat(dualTable(as.matrix(EOC_smry_B), align='rcc',
		caption='2017 EOC Median and Mean Student Growth Percentile by Content Area.'))
```
...
```{r, results='asis', echo=FALSE, SIMEX_SumEOG}
	EOG_SIMEX_smry <- Georgia_SGP@Data[CONTENT_AREA %in% GL_subjects & YEAR=='2017'][,list(MEDIAN=median(as.numeric(SGP_SIMEX_RANKED), na.rm=TRUE), MEAN=round(mean(SGP_SIMEX_RANKED, na.rm=TRUE), 1)), by=c('CONTENT_AREA', 'GRADE')][!is.na(MEDIAN)]
  ...
  cat(dualTable(as.matrix(EOG_SIMEX_smry_B), title="", n.cgroup=c(1, dim(EOG_SIMEX_smry_B)[2]-1), cgroup=c("", "Grades"),
  	align=paste(rep('r', dim(EOG_SIMEX_smry_B)[2]), collapse=''),
		caption='SIMEX Corrected EOG Median (Mean) Student Growth Percentile by Grade and Content Area for 2017'))
```


```{r, results='asis', echo=FALSE, SIMEX_SumEOC}
	EOC_SIMEX_smry <- Georgia_SGP@Data[CONTENT_AREA %in% EOC_subjects & YEAR=='2017'][,list("Median SGP"=median(as.numeric(SGP_SIMEX_RANKED), na.rm=TRUE), "Mean SGP"=round(mean(SGP_SIMEX_RANKED, na.rm=TRUE), 1)), by='CONTENT_AREA']
  ...
  cat(dualTable(as.matrix(EOC_SIMEX_smry_B), align='rcc',
		caption='SIMEX Corrected EOC Median (Mean) Student Growth Percentile by Content Area for 2017'))
```
...
```{r, results="asis", echo=FALSE, ELA_Bubble}
		placeFigure(
			files = "../img/Bubble_Plots/2017/State/Style_1/Georgia_2017_ELA_State_Bubble_Plot_(Prior_Achievement).png",
			caption = "School-level Bubble Plots for Georgia: ELA, 2016-2017.")
```
...
```{r, results="asis", echo=FALSE, Math_Bubble}
		placeFigure(
			files = "../img/Bubble_Plots/2017/State/Style_1/Georgia_2017_Mathematics_State_Bubble_Plot_(Prior_Achievement).png",
			caption = "School-level Bubble Plots for Georgia: Mathematics, 2016-2017.")
```
...
```{r, cache=TRUE, echo=FALSE, include=FALSE}
	sch.msgp <- Georgia_SGP@Summary$SCHOOL_NUMBER$SCHOOL_NUMBER__YEAR__SCHOOL_ENROLLMENT_STATUS[YEAR %in% c('2015', '2016', '2017') & !is.na(SCHOOL_NUMBER) & MEDIAN_SGP_COUNT > 14] # Last 3 YEARs
	sch.msgp.subj <- Georgia_SGP@Summary$SCHOOL_NUMBER$SCHOOL_NUMBER__CONTENT_AREA__YEAR__SCHOOL_ENROLLMENT_STATUS[YEAR %in% c('2015', '2016', '2017') & !is.na(SCHOOL_NUMBER) & MEDIAN_SGP_COUNT > 14] # Last 3 YEARs
	sch.msgp.grd <- Georgia_SGP@Summary$SCHOOL_NUMBER$SCHOOL_NUMBER__CONTENT_AREA__YEAR__GRADE__SCHOOL_ENROLLMENT_STATUS[YEAR == '2017' & !is.na(SCHOOL_NUMBER) & MEDIAN_SGP_COUNT > 14]
	...
	## 2017 by grade - School_Grade_EOG_Correlations
...
```{r, results='asis', echo=FALSE, School_Cor_Grand}
	...
	n_row_group <- NULL
	row_group <- NULL
	tmp_capt <- "2015 to 2017 Correlations between Mean Prior Standardized Scale Score and Aggregate SGPs - (Combined Subjects)"
...
```{r, results='asis', echo=FALSE, School_EOG_Correlations}
	...
	tmp.cap <- "2015 to 2017 School Level EOG Correlations between Mean Prior Standardized Scale Score and Aggregate SGPs by Content Area."	
...
```{r, results='asis', echo=FALSE, School_EOC_Cor}
	...
	tmp.cap <- "2015 to 2017 School Level EOC Correlations between Mean Prior Standardized Scale Score and Aggregate SGPs by Content Area."
	tmp_tbl$CONTENT_AREA <- sapply(tmp_tbl$CONTENT_AREA, capwords, USE.NAMES=FALSE)
	tmp_tbl$CONTENT_AREA[duplicated(tmp_tbl$CONTENT_AREA)] <- ""
	setnames(tmp_tbl, sapply(names(tmp_tbl), capwords))
