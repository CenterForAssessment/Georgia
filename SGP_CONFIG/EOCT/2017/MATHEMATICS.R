...
COORDINATE_ALGEBRA_2017.config <- list(
  COORDINATE_ALGEBRA.2017 = list( #15
    ...
    sgp.exclude.sequences = data.table(VALID_CASE = 'VALID_CASE', CONTENT_AREA=c('COORDINATE_ALGEBRA', 'MATHEMATICS'),
                                       YEAR=c('2016', '2016'), GRADE=c('EOCT', '8')),
...
GEOMETRY_2017.config <- list(
  ...
  GEOMETRY.2017 = list( #36
    sgp.content.areas=c('MATHEMATICS', 'MATHEMATICS', 'GEOMETRY'),
    sgp.panel.years=c('2014', '2015', '2017'),
    sgp.grade.sequences=list(c(7, 8, 'EOCT')),
    sgp.panel.years.within=c('LAST_OBSERVATION', 'LAST_OBSERVATION', 'FIRST_OBSERVATION'),
    sgp.exact.grade.progression=TRUE,
    sgp.exclude.sequences = data.table(VALID_CASE = 'VALID_CASE', CONTENT_AREA=c('ALGEBRA_I','GEOMETRY'),
